  ...
  const analyzeReceipt=async()=>{
    ...
    try{
      const raw=await callClaude({
        system:"You are a grocery receipt parser specialized in Meijer store receipts. Analyze this receipt image and extract every food/grocery item purchased. Return ONLY a valid JSON array. No markdown, no preamble.\n\nRULES:\n1. DUPLICATE HANDLING: If the same item appears multiple times, combine into one object with summed qty. If an item has quantity printed on the receipt (e.g. '2 @ $1.99'), use that quantity.\n2. UNIT RULES: Bananas/grapes→bunch. Milk→gallon. Eggs→dozen. Bread→loaf. Meat/fish by weight→lb. Ice cream/frozen→container. Produce bags→bag. Canned goods→can. Bottles→bottle. Multi-packs→count. Default→each.\n3. LOCATION RULES (MUST follow): Protein/Meat/Seafood/Poultry/Pork/Beef/Fish→Freezer. Dairy/Eggs/Fresh Produce/Deli/Condiments/Juice→Fridge. Canned/Dry/Spices/Grains/Baking/Snacks/Frozen packaged→Pantry or Freezer.\n4. MEIJER SPECIFICS: Ignore PLU numbers, barcodes, discount lines ('SAVE', 'MPERKS', 'COUPON'), tax lines, and totals. Clean brand names (e.g. 'MEIJER ORG' → 'Organic [Item]').\n5. CONFIDENCE: Rate each item high/medium/low based on how clearly it appears on the receipt.\n\nEach object: {name, qty(number), unit, category(Protein|Produce|Dairy|Pantry|Grains|Spices|Frozen|Condiments|Other), location(Freezer|Fridge|Pantry), isProtein(boolean), price(string), confidence(high|medium|low)}. Skip non-food items.",

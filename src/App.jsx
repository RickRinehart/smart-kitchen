                    ...
                    <div key={i} style={{background:item.selected?"#1f2a40":C.card,border:"1px solid "+(item.selected?C.borderLight:C.border),borderRadius:10,padding:"10px 14px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:item.selected?8:0,cursor:"pointer"}} onClick={()=>setScanResults(p=>p.map((si,sii)=>sii===i?{...si,selected:!si.selected}:si))}>
                        ...
                        <div style={{flex:1}}>
                          ...
                          <div style={{fontSize:11,color:C.muted,fontFamily:FM,display:"flex",alignItems:"center",gap:4,flexWrap:"wrap"}}><input type="number" style={{width:50,fontSize:11,border:"1px solid #555",borderRadius:3,padding:"1px 4px",background:"#1e1e1e",color:"#ffffff"}} value={item.qty} onClick={e=>e.stopPropagation()} onFocus={e=>e.stopPropagation()} onChange={e=>{const updated=[...scanResults];updated[idx]={...updated[idx],qty:e.target.value};setScanResults(updated)}} /><input style={{width:60,fontSize:11,border:"1px solid #555",borderRadius:3,padding:"1px 4px",background:"#1e1e1e",color:"#ffffff"}} value={item.unit} onClick={e=>e.stopPropagation()} onFocus={e=>e.stopPropagation()} onChange={e=>{const updated=[...scanResults];updated[idx]={...updated[idx],unit:e.target.value};setScanResults(updated)}} /> · {item.category} · {item.location||"Pantry"}{item.price?" · "+item.price:""}</div>
                        </div>

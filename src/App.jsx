...
Keep responses concise — 2-4 sentences max unless explaining a feature. Use plain language. No bullet points unless listing steps.`;
  ...
  useEffect(()=>{
    ...
    const doAccept=async()=>{
      ...
      try{
        const res=await fetch("/api/accept-invite",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({invite_code:acceptCode,invitee_uid:user.id,invitee_email:user.email,invitee_name:user.user_metadata?.full_name||""})});

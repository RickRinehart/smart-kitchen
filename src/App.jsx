...
export default function SmartKitchen(){
  ...
  const [scanResults,setScanResults]=useState(null);const [changeMealModal,setChangeMealModal]=useState(null);const [changeMealRequest,setChangeMealRequest]=useState("");const [changeMealLoading,setChangeMealLoading]=useState(false);
  ...
  const fetchRecipes=async()=>{
    ...
    try{
      ...
      const raw=await callClaude({
        ...
        prompt:"Proteins: "+proteins+". Saute blend bags: "+(blendItem?.qty||0)+". Make 4 simple weeknight dinners for 3 people. Vary proteins — include beef and pork not just chicken.",
      });
      const s=raw.indexOf("["),e=raw.lastIndexOf("]");

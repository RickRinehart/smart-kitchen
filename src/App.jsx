                  ...
                  {mealPlan.map((day,i)=>(
                    <div key={i} style={{background:day.quickMeal?"#1a1a2e":C.card,border:"1px solid "+(day.quickMeal?"#f59e0b":C.border),borderRadius:12,padding:15,display:"grid",gridTemplateColumns:"80px 1fr auto",gap:14,alignItems:"start"}}>
                      ...
                      <div>
                        ...
                        <div style={{fontFamily:FD,fontSize:16,marginBottom:7,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><a href={getRecipeUrl(day.meal)} target="_blank" rel="noopener noreferrer" style={{color:C.accent,cursor:"pointer",fontFamily:FD,fontSize:16,textDecoration:"none"}}>🔍 {day.meal}</a><span style={{fontFamily:FM,fontSize:10,color:C.muted,opacity:0.7}}>tap for full recipe →</span>{day.ingredients&&day.ingredients.length>0&&<div style={{width:"100%",marginTop:6,padding:"8px 10px",background:"rgba(255,255,255,0.05)",borderRadius:6,fontSize:11,fontFamily:FM}}><div style={{fontWeight:600,marginBottom:4,color:C.muted}}>INGREDIENTS</div>{day.ingredients.map((ing,ii)=><div key={ii} style={{color:C.text,marginBottom:2}}>· {ing}</div>)}</div>}</div>

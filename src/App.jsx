                  ...
                  {mealPlan.map((day,i)=>(
                    <div key={i} style={{background:day.quickMeal?"#1a1a2e":C.card,border:"1px solid "+(day.quickMeal?"#f59e0b":C.border),borderRadius:12,padding:15,display:"grid",gridTemplateColumns:"80px 1fr auto",gap:14,alignItems:"start"}}>
                      ...
                      <div>
                        ...
                        <div style={{fontFamily:FD,fontSize:16,marginBottom:7}}><span onClick={()=>setExpandedDay(expandedDay===i?null:i)} style={{color:C.accent,cursor:"pointer",fontFamily:FD,fontSize:16,marginBottom:7}}>🔍 {day.meal}</span>{expandedDay===i&&day.ingredients&&day.ingredients.length>0&&<div style={{marginTop:8,padding:"10px",background:"rgba(255,255,255,0.05)",borderRadius:6,fontSize:11,fontFamily:FM}}><div style={{fontWeight:600,marginBottom:4,color:C.muted}}>INGREDIENTS</div>{day.ingredients.map((ing,ii)=><div key={ii} style={{color:C.text,marginBottom:2}}>· {ing}</div>)}</div>}</div>

                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:14}}>
                  {recipes.filter(r=>(recipeRatings[r.name]||0)!==1||(recipeRatings[r.name]||0)===1).map(r=>{
                    ...
                    <div key={r.id} style={{background:C.card,border:"1px solid "+C.border,borderRadius:14,padding:18,cursor:"pointer",transition:"all 0.15s",opacity:rating===1?0.5:1}}
                      ...
                      <div style={{marginTop:10,display:"flex",gap:8}}><button onClick={e=>{e.stopPropagation();setSwapRecipeModal(r);setSwapRecipeRequest("");}} style={{flex:1,padding:"8px",borderRadius:8,border:"1px solid "+C.border,background:C.surface,color:C.text,fontFamily:FM,fontSize:12,cursor:"pointer"}}>✦ Swap Recipe</button><button onClick={e=>{e.stopPropagation();setPhotoPromptMeal(r.name);}} style={{padding:"8px 12px",borderRadius:8,border:"1px solid "+C.border,background:C.surface,color:C.muted,fontFamily:FM,fontSize:12,cursor:"pointer"}} title="Add photo">📸</button></div>
                    </div>
                  )})}
                </div>
              </div>
                  ...
                  {mealPlan.map((day,i)=>(
                    <div key={i} style={{background:C.card,border:"1px solid "+(day.quickMeal?"#f59e0b":C.border),borderRadius:seniorMode?16:12,padding:seniorMode?18:12}}>
                      ...
                      <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                        ...
                        <button onClick={()=>setPhotoPromptMeal(day.meal)} style={{background:"transparent",border:"1px solid "+C.border,borderRadius:seniorMode?10:6,color:C.muted,cursor:"pointer",fontFamily:FM,fontSize:seniorMode?18:11,padding:seniorMode?"12px 14px":"8px 10px",flexShrink:0}} title="Add photo">📸</button>
                    ...
                    <div key={d.id} style={{background:C.card,border:"1px solid "+(rating===1?C.red+"66":rating>=4?C.accent+"66":C.border),borderRadius:14,padding:18,cursor:"pointer",transition:"all 0.15s",opacity:rating===1?0.5:1}}
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        ...
                        <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
                          ...
                          <button onClick={e=>{e.stopPropagation();setPhotoPromptMeal(d.name);}} style={{background:"transparent",border:"1px solid "+C.border,borderRadius:4,color:C.muted,fontFamily:FM,fontSize:10,padding:"3px 8px",cursor:"pointer"}} title="Add photo">📸</button>
                        </div>
                      </div>
                    </div>
                  )})}

...
Keep responses concise — 2-4 sentences max unless explaining a feature. Use plain language. No bullet points unless listing steps.`;
  ...
  const fetchRecipes=async()=>{
    ...
    try{
      ...
      const raw=await callClaude({
        system:"Return ONLY a JSON array of 4 dinner recipes. No other text. Start with [ end with ]. Each object has exactly these keys: id (number), name (string), time (string like 30 min), difficulty (Easy or Medium or Hard), description (one short sentence), usesFromInventory (array of 3 strings max), missingIngredients (array of strings), instructions (array of 4 short strings — EVERY step must restate the specific measured amount for each ingredient it uses, never just the name alone), cellarItem (string or null — the exact name of a cellar/bar item from the provided cellar list ONLY if it is genuinely used in this recipe, otherwise null). Keep all strings short."+(fs?" CRITICAL DIETARY RULES — these are HARD STOPS, never violate them: "+fs:""),
  ...
  const openMealPlanRecipe=async(day)=>{
    ...
    try{
      const raw=await callClaude({
        ...
        prompt:`Give a simple home recipe for "${day.meal}". Full inventory on hand: ${invList}.`+(knownIngredients?` Use EXACTLY this ingredient list with its measurements — do not invent a different one: ${knownIngredients}.`:"")+` Return JSON: {name,description,time,difficulty,instructions:[4-6 short strings — EVERY step MUST restate the specific measured amount for each ingredient it uses (e.g. "Stir in 1 cup shredded cheddar and 2 tbsp cream cheese until melted", NOT "stir in cheese"). Never reference an ingredient by name alone without its quantity.],usesFromInventory:[items from inventory used],missingIngredients:[items NOT in inventory]}`,
        maxTokens:700
      });

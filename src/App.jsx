...
const fiberTargetFor=(p)=>{
  ...
  else if(sex==="Female") base=(age&&age>50)?21:25;
  else base=25;
  const phase=p&&p.bariatricPhase;
  if(phase==="Liquid") base=Math.min(base,5);
  else if(phase==="Pureed") base=Math.min(base,10);
  else if(phase==="Soft") base=Math.min(base,15);
  const meds=((p&&p.medications)||[]).map(m=>(m.name||"").toLowerCase());
  if(meds.some(m=>m.includes("oxycodone")||m.includes("hydrocodone")||m.includes("morphine"))) base+=5;
  const sups=((p&&p.supplements)||[]).map(s=>(s.name||"").toLowerCase());
  const onFiberSupplement=sups.some(s=>s.includes("benefiber")||s.includes("metamucil")||s.includes("psyllium")||s.includes("methylcellulose")||s.includes("inulin")||s.includes("fiber"));
...
// Live alcohol-department sale check for Pair a Drink -- so a pairing recommendation can
// mention a genuinely fitting bottle that's actually on sale right now, not just whatever's
// already in the cellar. Queried fresh each time rather than relying on a possibly-stale pulled
// snapshot, so it reflects the current week's ads without requiring a manual pull first.
const fetchAlcoholSaleAds=async(userId)=>{
  if(!userId) return [];
  const {data:markets}=await supabase.from('user_preferred_markets').select('partner_store_id').eq('user_id',userId);
...
const FEATURE_ANNOUNCEMENTS=[
  ...
  {
    key:"weeklyAdDatesStore",
    title:"New: Smarter Weekly Ad Scanning — Dates & Store Tracked",
    intro:(name)=>`Hi ${name}! 🏷 The Weekly Ad Scanner got smarter.\n\nScanning an ad now reads the store and the sale's effective dates automatically — with a **This Week** or **Next Week Preview** choice you confirm before saving. Preview items save now but stay quiet until their week actually starts, so you can plan ahead without them showing up as active too early. You can even have this week's ad and next week's preview loaded at the same time.\n\nWant me to show you?`,
    quickReplies:["Show me!","How does it work?","Maybe later"],
    tab:"mealPlan",
    digest:"**Weekly Ad Scanner — dates & store tracking** — confirm the sale's effective dates and store, with Preview items staying inactive until their week arrives"
  },
  {
    key:"smarterWayToShop",
    title:"New: Smarter Way to Shop",
    intro:(name)=>`Hi ${name}! 🛒 Something new just landed — **Smarter Way to Shop**.\n\nIt compares your preferred grocery stores' weekly ads against your own inventory, so you can see what's currently cheaper than what you usually pay — no manual ad-scanning needed. It also flags Deep Discount Alerts when something you actually buy shows up as a steep, limited-quantity deal.\n\nIt's included if you're in your trial or already have the add-on ($5/mo as a Smart Kitchen member, or add it anytime from the banner in Meal Plan). Want me to show you where to find it?`,
...
function NutritionDashboard({familyProfiles,user,supabase,seniorMode,C,FM,FD,refreshKey,recallAlerts}){
  ...
  const targets={
    ...
    carbs_g:130,
    sat_fat_g:20,
    fiber_g:fiberTargetFor(activeP),
...
export default function SmartKitchen({ tier="free", can={}, onUpgrade=()=>{}, user=null, viewerRole=null, isAdmin=false, onShowGuestViewer=null }){
  ...
  useEffect(()=>{
    // If a queued item was deleted from inventory entirely (not just restocked), its name would
    // otherwise linger in the queue forever with nothing to ever clear it.
    setRestockQueue(prev=>{
      const valid=prev.filter(name=>inventory.some(i=>i.name.toLowerCase()===name.toLowerCase()));
      if(valid.length!==prev.length){try{localStorage.setItem("sk_restockQueue",JSON.stringify(valid));}catch{}return valid;}
      return prev;
    });
  },[inventory]);
  ...
  const [budgetAmount,setBudgetAmount]=useState(()=>localStorage.getItem("sk_budgetAmount")||"");
  const [budgetPeriod,setBudgetPeriod]=useState(()=>localStorage.getItem("sk_budgetPeriod")||"Weekly");
  const [spendLog,setSpendLog]=useState(()=>{try{return JSON.parse(localStorage.getItem("sk_spendLog")||"[]");}catch{return [];}});
  const logSpend=(amount)=>{if(!amount||amount<=0)return;setSpendLog(prev=>{const next=[...prev,{date:new Date().toISOString(),amount}];try{localStorage.setItem("sk_spendLog",JSON.stringify(next));}catch{}return next;});};
  const periodDays={Daily:1,Weekly:7,Biweekly:14,Monthly:30}[budgetPeriod]||7;
  const periodSpend=spendLog.filter(s=>new Date(s.date)>new Date(Date.now()-periodDays*86400000)).reduce((sum,s)=>sum+s.amount,0);
  ...
  useEffect(()=>{try{localStorage.setItem("sk_fetchedRecipeCache",JSON.stringify(fetchedRecipeCache));}catch{}},[fetchedRecipeCache]);
  ...
  const fetchCurrentPricing=async()=>{
    ...
    try{
      const res=await fetch("/api/create-checkout-session?action=current-pricing");
      if(!res.ok)throw new Error("Pricing service returned "+res.status);
      const data=await res.json();
      setLivePricing(data.prices||[]);
      setPricingFetchedAt(data.fetchedAt);
    }catch(err){
      console.error("Pricing fetch error:",err);
      setPricingError("Couldn't load live pricing right now — please try again in a moment.");
    }
  ...
  const [showCuisinePanel,setShowCuisinePanel]=useState(false);
  const [expandedCuisine,setExpandedCuisine]=useState(null);
  const [cuisinePantryCache,setCuisinePantryCache]=useState({});
  const [cuisinePantryLoading,setCuisinePantryLoading]=useState(null);
  const [cuisinePantryTab,setCuisinePantryTab]=useState("pantry");
  const [cuisineAddedFeedback,setCuisineAddedFeedback]=useState({});
  const fetchCuisinePantry=async(cuisine)=>{
    if(cuisinePantryCache[cuisine]){return;}
    setCuisinePantryLoading(cuisine);
    try{
      const raw=await callClaude({
        system:"Return ONLY valid JSON, no markdown, no other text. Given a cuisine, list the ingredients a home cook stocking that cuisine's pantry would most commonly need, split into four categories.",
        prompt:"Cuisine: "+cuisine+". Return this exact JSON shape: {\"pantry\":[...50 shelf-stable pantry items, spices, condiments, sauces, grains, canned/dry goods -- most essential first],\"protein\":[...10 common proteins for this cuisine],\"produce\":[...10 common fruits/vegetables/herbs for this cuisine],\"dairy\":[...up to 10 common dairy items for this cuisine, fewer is fine if the cuisine uses little dairy]}. Short plain ingredient names only, no descriptions, no quantities.",
        maxTokens:1800
      });
      const txt=typeof raw==="string"?raw:Array.isArray(raw)?raw.map(r=>r.text||"").join(""):raw?.content?.[0]?.text||"";
      const clean=txt.replace(/```json|```/g,"").trim();
      const s=clean.indexOf("{"),e=clean.lastIndexOf("}");
      const parsed=JSON.parse(clean.slice(s,e+1));
      setCuisinePantryCache(prev=>({...prev,[cuisine]:{
        ...
        dairy:Array.isArray(parsed.dairy)?parsed.dairy:[]
      }}));
    }catch(err){
      showAlert("Could not load pantry list for "+cuisine+". Try again.");
    }
  ...
  const addCuisineItemToInventory=(itemName,category)=>{
    ...
    setInventory(prev=>[...prev,{id:Date.now()+Math.random(),name:itemName,qty:0,unit:"item",category:category||"Pantry",addedDate:new Date().toISOString().split("T")[0]}]);
    setCuisineAddedFeedback(prev=>({...prev,[itemName]:"inv"}));
  };
  const addCuisineItemToShopping=(itemName)=>{
    if(shopping.some(s=>(s.name||"").toLowerCase()===itemName.toLowerCase())){setCuisineAddedFeedback(prev=>({...prev,[itemName]:"shop"}));return;}
    setShopping(prev=>[...prev,{name:itemName,qty:1,unit:"item",category:"Pantry",checked:false,source:"Cuisine pantry list"}]);
    setCuisineAddedFeedback(prev=>({...prev,[itemName]:"shop"}));
  };
  ...
  const [chatBubblePos,setChatBubblePos]=useState(()=>{
    try{
      const s=localStorage.getItem("sk_chatBubblePos");
      if(!s) return {x:null,y:null};
      const pos=JSON.parse(s);
      if(pos.x==null||pos.y==null) return pos;
      const clamped={x:Math.max(8,Math.min(window.innerWidth-58,pos.x)),y:Math.max(8,Math.min(window.innerHeight-58,pos.y))};
      if(clamped.x!==pos.x||clamped.y!==pos.y){try{localStorage.setItem("sk_chatBubblePos",JSON.stringify(clamped));}catch{}}
      return clamped;
    }catch{return {x:null,y:null};}
  });
  useEffect(()=>{
    const reclamp=()=>{
      setChatBubblePos(prev=>{
        if(prev.x==null||prev.y==null) return prev;
        const clamped={x:Math.max(8,Math.min(window.innerWidth-58,prev.x)),y:Math.max(8,Math.min(window.innerHeight-58,prev.y))};
        if(clamped.x!==prev.x||clamped.y!==prev.y){try{localStorage.setItem("sk_chatBubblePos",JSON.stringify(clamped));}catch{}return clamped;}
        return prev;
      });
    };
    ...
    if(can.medicalCompliance){
      ...
      if(medProfiles.length>0){
        s+="MEDICAL+ PROFILES: "+medProfiles.map(p=>{
          ...
          if(p.medicalPlan||p.fiberTargetG) parts.push("Daily Fiber Target: "+fiberTargetFor(p)+"g \u2014 include fiber-rich ingredients (vegetables, whole grains, legumes, fruit) across meals to help reach this daily target");
...
- MISE EN PLACE SCALE WEIGHING: Whenever a Bluetooth scale is connected, every recipe's ingredient list (Meal Plan, Recipes tab, Make This, Desserts, and Family Recipes) shows a "⚖ Weigh" button next to each ingredient line. Tapping it tares the scale and shows the live weight color-coded against a target: blue while idle, yellow while below the target weight, green once the target is reached, red if over — visual confirmation of correct measurement, useful for teaching proper portioning technique. The target assumes standard spooned-and-leveled measurement, not packed/compressed (e.g. packed flour weighs more than spooned flour for the same "1 cup"). Tapping "✓ Log" confirms that ingredient's actual weighed amount. If that ingredient matches a bulk-tracked pantry item, the confirmed weighed amount is used for inventory deduction instead of the recipe's stated amount when "I Cooked This"/"I Made This" is tapped — a real weigh-in is more accurate than any recipe estimate. The "Cooked!"/"Made This" confirmation shows an itemized breakdown of which ingredients used a weighed amount versus an estimate. Family Recipes now has its own "I Made This — Update Inventory" button (added Sept 2026), sharing the same deduction logic as every other recipe surface.
- CURRENT SUBSCRIPTION PRICING: If asked what a tier costs, don't recite a number from memory — point the user to Settings → "Current Subscription Pricing" (near the top, next to the YouTube tutorials link). It pulls live prices directly from Stripe every time it's opened, so it's always accurate even if pricing changes later, unlike a number you might have memorized.
- WEEKLY AD SCANNER — DATES & STORE: Scanning a weekly ad now detects the store name and the sale's effective date range automatically, shown back on the review screen for the user to confirm or correct before saving — with a "This Week" vs. "Next Week Preview" toggle. Preview items save and display but stay inactive (excluded from Build Sale Meal Plan and any shopping suggestions) until their week actually arrives, and automatically activate on their start date with no manual step. Multiple weeks/stores can be loaded at once without overwriting each other — e.g. this week's Meijer ad and next week's preview can coexist.
...
- SMARTER WAY TO SHOP (a-la-carte add-on, $5/mo as a Smart Kitchen member — $10/mo standalone; included free during the 30-day trial): compares your preferred grocery stores' weekly ads against your own inventory and purchase history, surfacing items currently priced below what you usually pay — no manual ad-scanning required, unlike the free Weekly Ad Scanner which requires photographing one ad at a time. Also powers Deep Discount Alerts — a persistent top-of-app banner (separate from the Meal Plan tab banner) when something you actually buy shows up as a steep, often limited-quantity discount at one of your preferred stores. If the user doesn't have it yet, they can add it anytime via the banner in the Meal Plan tab, or select it when subscribing/upgrading.
...
Keep responses concise — 2-4 sentences max unless explaining a feature. Use plain language. No bullet points unless listing steps.`;
  ...
  const onFiles=async(files)=>{
    ...
    } catch(e){ showAlert("Ad scan failed: "+e.message); setScanStage("upload"); }
    setLoading(false);
  };
  ...
  const analyzeReceipt=async()=>{
    ...
    try{
      const raw=await callClaude({
        system:"You are a grocery receipt parser specialized in Meijer store receipts. Analyze this receipt image and extract EVERY item purchased — food AND non-food alike (household, cleaning, personal care, pet products are just as important to capture as groceries). Return ONLY a valid JSON array. No markdown, no preamble.\n\nRULES:\n1. DUPLICATE HANDLING: If the EXACT same product appears multiple times (same name, same price), combine into one object with summed qty. However if similar items appear with DIFFERENT SKUs or flavors (e.g. two different ice cream flavors), keep them SEPARATE but flag confidence as medium. If an item has quantity printed (e.g. '2 @ $1.99'), use that quantity.\n2. UNIT RULES: Bananas/grapes→bunch. Milk→gallon. Eggs→dozen. Bread→loaf. Meat/fish by weight→lb. Ice cream/frozen novelty→container. Produce bags (onions/potatoes)→bag. Canned goods→can. Bottles→bottle. Multi-packs→count. Default→each.\n3. LOCATION RULES (MUST follow exactly): Protein/Meat/Seafood/Poultry/Pork/Beef/Fish/Ice cream/Frozen→Freezer. Dairy/Eggs/Deli/Juice/Condiments/Dressings→Fridge. Fresh produce (bananas/apples/oranges/tomatoes/peppers)→Fridge. Bagged produce (onions/potatoes/carrots)→Pantry. Canned/Dry/Spices/Grains/Baking/Snacks/Beverages→Pantry. Household/Cleaning/Personal Care/Pet items→Pantry (no dedicated storage location exists for these yet).\n4. RECEIPT SPECIFICS: Ignore PLU numbers, discount lines (SAVE, MPERKS, COUPON, MFR), tax lines, subtotals and totals. Clean brand names (MEIJER ORG→Organic [Item], WB→Wright Brand). Works with any retailer.\n5. UPC CODES: If a 12-digit UPC barcode number is visible next to an item, capture it in the upc field. Set upc to null if not visible.\n6. CONFIDENCE: high=clearly readable name+price. medium=similar items or slightly unclear. low=guessed from partial text.\n7. NON-FOOD CATEGORIZATION: Household (paper towels, trash bags, light bulbs, batteries), Cleaning (dish soap, laundry detergent, all-purpose cleaner, disinfectant wipes), Personal Care (shampoo, toothpaste, deodorant, vitamins, OTC medicine), Pet (pet food, litter, pet treats/toys). isProtein is always false for non-food items.\n\nEach object: {name, qty(number), unit, category(Protein|Produce|Dairy|Pantry|Baking|Grains|Spices|Frozen|Condiments|Household|Cleaning|Personal Care|Pet|Other), location(Freezer|Fridge|Pantry), isProtein(boolean), price(string), upc(string|null), confidence(high|medium|low), expiryDays(number|null, estimated days until expiry: fresh meat=2, ground beef=2, pork=3, chicken=2, fish=1, milk=7, eggs=21, cheese=14, yogurt=10, fresh produce=5, bananas=5, bread=5, deli meat=5, frozen=180, canned=730, dry goods=365, condiments=180, null for pantry staples or household/cleaning/personal care/pet items with very long or indefinite shelf life)}.",
    ...
    } catch(e){ showAlert("Receipt scan failed: "+e.message); }
    setLoading(false);
  };
  ...
  const analyzePhoto=async()=>{
    ...
    try{
      ...
      setScanStage("review");
    } catch(e){ showAlert("Shelf scan failed: "+(e.message||"unknown error")+". If this keeps happening, try a clearer or smaller photo, or scan one item at a time."); }
    setLoading(false);
  };
  const analyzeWhiteboard=async()=>{
    ...
    } catch(e){ showAlert("List scan failed: "+e.message); }
    setLoading(false);
  };
  ...
  const commitScan=()=>{
    ...
    if(scanTotal>0) logSpend(scanTotal);
    setInventory(prev=>{
      const u=[...prev];
      chosen.forEach(si=>{
  ...
  // -- AI functions -----------------------------------------------------------
  // Shared cellar-cooking eligibility check — single source of truth so every recipe generation
  // path (Meal Plan, Recipes tab, Explore, Swap, Make This) respects the toggle and per-bottle
  // exclusion the same way, instead of duplicating this fetch/filter logic at each call site.
  const setCellarBottleExcluded=async(bottleId,excluded)=>{
    if(!user?.id) return;
    try{
      const {data}=await supabase.from("profiles").select("sc_cloud_data").eq("id",user.id).single();
      ...
      const updatedCellar=(parsed.cellar||[]).map(cb=>cb.id===bottleId?{...cb,excludeFromCooking:excluded}:cb);
      await supabase.from("profiles").update({sc_cloud_data:{...parsed,cellar:updatedCellar}}).eq("id",user.id);
      setCellarInvDisplay(prev=>Array.isArray(prev)?prev.map(pb=>pb.id===bottleId?{...pb,excludeFromCooking:excluded}:pb):prev);
    }catch(e){console.error("Cellar exclusion update error:",e);}
  };
  const getCellarCookingBlock=async()=>{
    if(!cellarCookingEnabled||!user?.id) return {block:"",schemaField:""};
    try{
      const {data}=await supabase.from("profiles").select("sc_cloud_data").eq("id",user.id).single();
      if(!data?.sc_cloud_data) return {block:"",schemaField:""};
      const parsed=typeof data.sc_cloud_data==="string"?JSON.parse(data.sc_cloud_data):data.sc_cloud_data;
      const eligible=(parsed.cellar||[]).filter(b=>!b.excludeFromCooking&&(b.remaining_pct??100)>5&&cellarCookingUsesFor(b.category));
      if(eligible.length===0) return {block:"",schemaField:""};
      const list=eligible.map(b=>b.name+" ("+b.category+" — "+cellarCookingUsesFor(b.category).join("/")+")").join(", ");
      return {
        block:" Cellar/bar items optionally available for cooking (ONLY use one if it genuinely fits the dish, e.g. deglazing, braising, marinade, flambé — never force it): "+list+".",
        schemaField:",cellarItem(string or null — the exact name of a cellar/bar item from the provided cellar list ONLY if it is genuinely used in this recipe, otherwise null)"
      };
    }catch{return {block:"",schemaField:""};}
  };
  const fetchRecipes=async()=>{
    ...
    try{
      ...
      const cellarBlock=cellarInfo.block;
      const raw=await callClaude({
        system:"Return ONLY a JSON array of 4 dinner recipes. No other text. Start with [ end with ]. Each object has exactly these keys: id (number), name (string), time (string like 30 min), difficulty (Easy or Medium or Hard), description (one short sentence), servings (number, default 4), ingredients (array of {name,qty,unit} objects — qty is a NUMBER, unit is a short string like \"cup\",\"tsp\",\"tbsp\",\"oz\",\"lb\" or \"\" for countable items like eggs), usesFromInventory (array of 3 strings max), missingIngredients (array of strings), instructions (array of 4 short strings — CRITICAL: every step that references a measured ingredient MUST use a {{ing:N}} placeholder instead of typing a number, where N is that ingredient's zero-based index in the ingredients array. Never type a literal quantity for a measured ingredient — always use the placeholder.), cellarItem (string or null — the exact name of a cellar/bar item from the provided cellar list ONLY if it is genuinely used in this recipe, otherwise null). Keep all strings short."+(fs?" CRITICAL DIETARY RULES — these are HARD STOPS, never violate them: "+fs:""),
        prompt:"Proteins: "+proteins+". Saute blend bags: "+(blendItem?.qty||0)+". Make 4 simple weeknight dinners for 3 people. Vary proteins — include beef and pork not just chicken. Pantry/fridge inventory (match case-insensitively): "+inventory.filter(i=>(parseFloat(i.qty)??1)>0).map(i=>String(i.name||"")).filter(Boolean).join(", ")+". For missingIngredients only list items NOT in that inventory."+cellarBlock+(fs?" ENFORCE these dietary rules in every recipe: "+fs:""),
      });
      const s=raw.indexOf("["),e=raw.lastIndexOf("]");
  ...
  const buildSaleMealPlan=async()=>{
    ...
    } catch(err){ showAlert("Could not build sale meal plan: "+err.message); }
    setLoading(false);
  };
  const printLabels=()=>{
    const selectedItems=[];
  ...
  // Diet plans meaning "this person eats a wholesale different plate, not just a modified version
  // of the shared meal" -- for these, plating strips the shared meal down to protein only rather
  // than trying to make the household's starch/veg sides diet-compliant. Extend this list if more
  // all-meat/elimination-style plans get added later.
  const DIVERGENT_DIET_PLANS=["Carnivore"];
  const filterComponentsForDiet=(components,profile)=>{
    if(!profile||!DIVERGENT_DIET_PLANS.includes(profile.medicalPlan)) return components;
    return components.filter(c=>c.category==="protein");
  };
  ...
  const sendToDelivery=async()=>{
    if(!shopping.length){showAlert("Generate a shopping list first.");return;}
    ...
    const isOtherStore=instacartStore==="other"||!instacartStore;
    window.open(isOtherStore?"https://www.instacart.com/store":"https://www.instacart.com/store/"+instacartStore,"_blank");
    setInstacartLoading(false);
    setInstacartCopyCue({copied,listText,isOtherStore});
  };
  ...
  const shareRecipes=async()=>{
    ...
    if(!recipes.length){showAlert("Select at least one recipe to share.");return;}
    ...
    try{
      ...
      if(d.success){
        ...
        showAlert("Could not create share link: "+(d.error||"Please try again."));
      }
    }catch(e){
      showAlert("Could not share recipes: "+e.message);
    }
  ...
  const importSharedRecipes=async()=>{
    ...
    if(code.length<6){showAlert("Please enter a valid share code.");return;}
    ...
    try{
      ...
      if(d.success&&d.share){
        ...
        showAlert(d.error||"Recipe collection not found.");
      }
    }catch(e){
      showAlert("Could not fetch recipes: "+e.message);
    }
  ...
  const confirmImport=(recipes)=>{
    ...
    setImportCode("");
    setImportResult(null);
    showAlert(total+" recipe"+(total>1?"s":"")+" added to your Smart Kitchen!");
  };

  const planOccasionMeal=async()=>{
    if(!occasionState.eventType){
      showAlert("Please select an occasion type first.");
      return;
    }
    ...
    try{
      ...
      showAlert("Could not generate occasion meal: "+err.message.slice(0,80)+". Please try again.");
  ...
  const scheduleOccasionMeal=()=>{
    ...
    if(inCurrentWeek&&mealPlan.length>0){
      ...
      if(dayIdx>=0){
        ...
        showAlert("Meal plan updated! "+chosenDay+"'s dinner is now "+occasionResult.meal+".");
    ...
    showAlert("Added to Google Calendar for "+chosenDay+", "+new Date(occasionDate+"T12:00:00").toLocaleDateString()+"!");
    setShowOccasionPlanner(false);
    setOccasionStep("form");
    setOccasionResult(null);
    setOccasionDate("");
  ...
  useEffect(()=>{
    ...
    const doAccept=async()=>{
      ...
      try{
        ...
        if(d.success){
          if(d.role==="manager"){setIsManager(true);setManagerOwnerUid(d.owner_uid);showAlert("You are now a Manager on "+(d.owner_name||"this")+" account. You can edit their meal plan and inventory.");}
          else{showAlert("You are now a Viewer on "+(d.owner_name||"this")+" account. You can view their meal plan and inventory.");}
        }else{
          showAlert("Could not accept invitation: "+(d.error||"Unknown error"));
        }
      }catch(e){
        showAlert("Error accepting invitation. Please try again.");
      }
      setAcceptCode("");
  ...
  const startListening=()=>{
    ...
    if(!SpeechRec){showAlert("Voice input requires Chrome on Android or desktop.");return;}
  ...
  const quickMealForDay=async(dayIdx)=>{
    ...
    } catch(err){ showAlert("Could not get quick meal: "+err.message); }
    setLoading(false);
  };
  ...
  const clearQuickMeal=async(dayIdx)=>{
    ...
    try{
      ...
      setMealPlan(prev=>prev.map((d,i)=>i===dayIdx?newDay:d));
    } catch(err){ showAlert("Could not restore meal: "+err.message); }
    setLoading(false);
  };
  ...
  const regenerateDay=async(dayIdx)=>{
    ...
    try{
      ...
      setMealPlan(prev=>prev.map((d,i)=>i===dayIdx?newDay:d));
    } catch(err){ showAlert("Could not regenerate meal: "+err.message); }
    setLoading(false);
  };
  ...
  const confirmMadeIt=(day,sides,subs)=>{
    ...
    if(day.isLeftover){
      // A leftover meal is a reheat, not a cook -- it doesn't consume fresh protein, the sauté
      // blend, or any recipe ingredients. Deduct 1 serving from the matching leftover inventory
      // item instead of running the normal deduction logic below (previously isLeftover was
      // recorded in history but never actually acted on, so "Made It" on a leftover day silently
      // did nothing to the leftover's own serving count).
      const dayMealLower=(day.meal||"").trim().toLowerCase();
      setInventory(prev=>prev.map(item=>{
        if(!item.isLeftover) return item;
        const nameLower=item.name.toLowerCase();
        if(nameLower===dayMealLower||nameLower.includes(dayMealLower)||dayMealLower.includes(nameLower)){
          return{...item,qty:Math.max(0,(parseFloat(item.qty)||0)-1)};
        }
        return item;
      }));
    }else{
  ...
  const genShopping=async()=>{
    ...
    try{
      ...
      setTab("shopping");
    } catch(err){ showAlert("Could not generate shopping list: "+err.message); }
    setLoading(false);
  };
  ...
  const fetchDesserts=async(qOverride,useInvOverride)=>{
    ...
    try{
      ...
      const cellarInfo=await getCellarCookingBlock();
      const raw=await callClaude({
        system:"Return ONLY a JSON array of 4 dessert recipes. No other text. Start with [ end with ]. Each object: {id(number),name(string),time(string),difficulty(Easy|Medium|Hard),category(Baked|No-Bake|Quick-Treat),description(one sentence),servings(number, default 4),ingredients(array of {name,qty,unit} objects — qty is a NUMBER, unit is a short string like \"cup\",\"tsp\",\"tbsp\",\"oz\",\"lb\" or \"\" for countable items),steps(array of 3 short strings — CRITICAL: every step referencing a measured ingredient MUST use a {{ing:N}} placeholder instead of typing a number, where N is that ingredient's zero-based index in ingredients. Never type a literal quantity.),usesFromInventory(array of ingredient names from inventory),missingIngredients(array of items NOT in inventory)"+cellarInfo.schemaField+"}."+(fs?" CRITICAL DIETARY RULES — HARD STOPS: "+fs:""),
        prompt:"Full pantry/fridge inventory: "+invList+"."+queryPart+inventoryPart+cellarInfo.block+" Suggest 4 desserts. missingIngredients must ONLY list items NOT in the inventory above."+(fs?" ENFORCE these dietary rules: "+fs:""),
      });
      const s=raw.indexOf("["),e=raw.lastIndexOf("]");
  ...
  // Swaps two days' meals in place -- everything about the meal (name, ingredients, protein,
  // sauté usage, leftover/busy-night/occasion flags, shopping needs) moves with it, but the `day`
  // field itself (Monday/Tuesday/etc.) stays fixed at each position, so this is a pure trade of
  // which day gets which meal, not a regeneration of either one.
  const swapDays=(indexA,indexB)=>{
    if(indexA===indexB) return;
    setMealPlan(prev=>{
      const next=[...prev];
      const dayLabelA=next[indexA].day;
      const dayLabelB=next[indexB].day;
      next[indexA]={...prev[indexB],day:dayLabelA};
      next[indexB]={...prev[indexA],day:dayLabelB};
      return next;
    });
    setSwapDayModal(null);
  };
        ...
        if(data?.sc_cloud_data){
          ...
          if(bottle){
            ...
            if(prevPct>=75&&newPct<75){
              setShopping(prev=>{
                const alreadyOn=prev.some(e=>(e.name||"").toLowerCase()===bottle.name.toLowerCase());
                if(alreadyOn) return prev;
                return [...prev,{name:bottle.name,qty:1,unit:"bottle",category:"Smart Cellar",checked:false,source:"Smart Cellar — running low"}];
              });
            }
          }
        }
  ...
  const printRecipeCard=(recipe,photo,currentServings)=>{
    const scale=currentServings?currentServings/(recipe.servings||4):1;
    ...
    const ings=(recipe.ingredients||[]).filter(i=>i&&(typeof i==="object"?i.name:i.trim())).map(i=>"<div class='ingredient'>&#8226; "+scaleIngredientDisplay(i,scale)+"</div>").join("");
    const steps=(recipe.instructions||recipe.steps||[]).filter(s=>s&&(typeof s==="object"?s.text:s.trim())).map((s,i)=>"<div class='step'><span class='step-num'>"+(i+1)+".</span><span>"+renderStepText(s,recipe.ingredients,scale)+"</span></div>").join("");
    const meta="<span>&#9201; "+(recipe.time||"~30 min")+"</span><span>"+(recipe.difficulty||"")+"</span>"+(currentServings?"<span>Serves "+currentServings+"</span>":"");
  ...
  const pushToCalendar=async()=>{
    ...
    try{
      ...
      if(!w){showAlert("Please allow popups for this site, then tap Calendar again.");return;}
      if(urls.length>1){
        setTimeout(()=>{
          showConfirm("Dinner 1 of "+urls.length+" opened in Google Calendar.\n\nTap Confirm to open the next dinner, or Cancel to stop.",()=>{
            let i=1;
            const openNext=()=>{
              if(i>=urls.length){showAlert("✅ All "+urls.length+" dinners sent to Google Calendar!");return;}
              window.open(urls[i],"_blank");
              i++;
              if(i<urls.length) setTimeout(()=>{
                showConfirm("Dinner "+i+" of "+urls.length+" opened.\n\nTap Confirm to continue.",()=>openNext());
              },800);
              else setTimeout(()=>showAlert("✅ All "+urls.length+" dinners sent to Google Calendar!"),800);
            };
            openNext();
          });
        },1500);
      } else {
        setTimeout(()=>showAlert("✅ Dinner added to Google Calendar!"),800);
      }
    } catch(e){ showAlert("Calendar push failed: "+e.message); }
  };
...
{!isViewer&&<button style={{...bBtn("primary"),width:"100%",marginBottom:8}} onClick={async()=>{
  ...
  showAlert(ok?"Data saved to cloud ✓":"Sync failed — check connection");
...
{!isViewer&&<button style={{...bBtn("ghost"),width:"100%",border:"1px solid #b45309",color:"#b45309"}} onClick={async()=>{
  ...
  if(count>0){window.dispatchEvent(new Event("sk_cloud_loaded"));showAlert("Restored "+count+" items from backup ✓ Refresh to see changes.");}
  else showAlert("No backup found.");
}}>Restore from Backup</button>}
{isViewer&&<button style={{...bBtn("primary"),width:"100%"}} onClick={async()=>{
  try{
    const gv=JSON.parse(localStorage.getItem("sk_guest_viewer")||"{}");
    const code=gv.code||"";
    if(!code){showAlert("No family code found. Try joining again from the welcome screen.");return;}
    const res=await fetch("/api/viewer-data",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({code})});
    const result=await res.json();
    if(!result.success){showAlert(result.error||"Could not refresh. Check connection.");return;}
...
<button style={{...bBtn("ghost"),width:"100%",marginTop:8,border:"1px solid #7c3aed",color:"#4a1d96"}} onClick={()=>{setShowSettings(false);setShowJoinViewer(true);}}>&#128065; Join as Viewer (enter family code)</button>
    ...
    {showWizard&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{background:C.surface,borderRadius:16,padding:28,maxWidth:440,width:"100%",border:"1px solid "+C.border,maxHeight:"90vh",overflowY:"auto"}}>
            {wizardStep===-3&&(<div style={{padding:"8px 0"}}>
              ...
              <button
                style={{...bBtn("primary"),width:"100%",padding:seniorMode?18:14,fontSize:seniorMode?19:15,marginBottom:10}}
                onClick={()=>{setTrialName(wizardSignupName);setShowEmailGate(true);}}
              >
                Create Free Account →
              </button>
      ...
      {recallAlerts.length>0&&showRecallBanner&&(()=>{
        const hasCritical=recallAlerts.some(a=>a.severity==="critical");
        return (
          <div style={{background:hasCritical?"#4a0e0e":"#3a2a0e",borderBottom:"2px solid "+(hasCritical?"#ef4444":"#d97706"),padding:"12px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
      ...
      {showCuisinePanel&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowCuisinePanel(false)}><div style={{background:C.card,borderRadius:16,padding:24,width:420,maxWidth:"92vw",maxHeight:"85vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
          <div style={{fontFamily:FD,fontSize:20,fontWeight:700,color:C.text}}>🌍 Cuisines</div>
          <button onClick={()=>setShowCuisinePanel(false)} style={{background:"transparent",border:"none",color:C.muted,cursor:"pointer",fontSize:20,lineHeight:1}}>x</button>
        </div>
        <div style={{fontFamily:FM,fontSize:12,color:C.muted,marginBottom:14}}>Add or remove favorite cuisines, and tap one to see its most common pantry, protein, produce, and dairy items.</div>
        <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>
          {CUISINE_OPTIONS.map(c=>{const on=cuisinePrefs.includes(c);return(
            <button key={c} onClick={()=>{setCuisinePrefs(prev=>on?prev.filter(x=>x!==c):[...prev,c]);if(!on){setExpandedCuisine(c);fetchCuisinePantry(c);}else if(expandedCuisine===c){setExpandedCuisine(null);}}}
              style={{padding:"6px 12px",borderRadius:16,border:"1px solid "+(on?C.accent:C.border),background:on?C.accent+"22":"transparent",color:on?C.accent:C.text,fontFamily:FM,fontSize:12,cursor:"pointer"}}>
              {c}{on?" ✓":""}
            </button>
          );})}
        ...
        <div style={{display:"flex",gap:6,marginBottom:16}}>
          <input value={cuisineCustomInput} onChange={e=>setCuisineCustomInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&cuisineCustomInput.trim()){const val=cuisineCustomInput.trim();if(!cuisinePrefs.includes(val)){setCuisinePrefs(prev=>[...prev,val]);setExpandedCuisine(val);fetchCuisinePantry(val);}setCuisineCustomInput("");}}} placeholder="Add another cuisine..." style={{flex:1,background:C.surface,border:"1px solid "+C.border,borderRadius:6,padding:"6px 10px",color:C.text,fontFamily:FM,fontSize:12,outline:"none"}}/>
          <button onClick={()=>{const val=cuisineCustomInput.trim();if(val&&!cuisinePrefs.includes(val)){setCuisinePrefs(prev=>[...prev,val]);setExpandedCuisine(val);fetchCuisinePantry(val);setCuisineCustomInput("");}}} style={{...bBtn("primary"),padding:"6px 12px",fontSize:12}}>+ Add</button>
        </div>
        {cuisinePrefs.length>0&&<div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:8}}>
          {cuisinePrefs.map(c=>(
            <div key={c} style={{background:C.surface,borderRadius:10,overflow:"hidden"}}>
              <button onClick={()=>{const opening=expandedCuisine!==c;setExpandedCuisine(opening?c:null);setCuisinePantryTab("pantry");if(opening)fetchCuisinePantry(c);}} style={{width:"100%",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:"transparent",border:"none",cursor:"pointer",fontFamily:FM,fontSize:13,fontWeight:600,color:C.text}}>
                <span>{c}</span><span style={{color:C.muted,fontSize:11}}>{expandedCuisine===c?"▲ Hide items":"▼ Show items"}</span>
              </button>
              {expandedCuisine===c&&<div style={{padding:"0 14px 14px"}}>
                {cuisinePantryLoading===c?(
                  <div style={{fontFamily:FM,fontSize:12,color:C.muted,padding:"10px 0"}}>⏳ Finding common {c} ingredients...</div>
                ):cuisinePantryCache[c]?(<>
                  <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>
                    {[["pantry","Pantry"],["protein","Protein"],["produce","Produce"],["dairy","Dairy"]].map(([k,label])=>(
                      <button key={k} onClick={()=>setCuisinePantryTab(k)} style={{padding:"5px 10px",borderRadius:8,border:"1px solid "+(cuisinePantryTab===k?C.accent:C.border),background:cuisinePantryTab===k?C.accent+"22":"transparent",color:cuisinePantryTab===k?C.accent:C.muted,fontFamily:FM,fontSize:11,cursor:"pointer",fontWeight:600}}>{label} ({(cuisinePantryCache[c][k]||[]).length})</button>
                    ))}
                  </div>
                  <div style={{display:"flex",flexDirection:"column",gap:4,maxHeight:260,overflowY:"auto"}}>
                    {(cuisinePantryCache[c][cuisinePantryTab]||[]).map((item,idx)=>{
                      const haveIt=cuisineItemHaveIt(item);
                      const feedback=cuisineAddedFeedback[item];
                      return(<div key={idx} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,padding:"6px 8px",borderRadius:6,background:C.card}}>
                        <span style={{fontFamily:FM,fontSize:12,color:C.text,flex:1}}>{item}</span>
                        {haveIt?(
                          <span style={{fontFamily:FM,fontSize:10,color:"#22c55e",fontWeight:700,flexShrink:0}}>✓ Have it</span>
                        ):feedback==="inv"?(
                          <span style={{fontFamily:FM,fontSize:10,color:C.accent,fontWeight:700,flexShrink:0}}>✓ In inventory</span>
                        ):feedback==="shop"?(
                          <span style={{fontFamily:FM,fontSize:10,color:C.accent,fontWeight:700,flexShrink:0}}>✓ On shopping list</span>
                        ):(
                          <div style={{display:"flex",gap:4,flexShrink:0}}>
                            <button onClick={()=>addCuisineItemToInventory(item,cuisinePantryTab==="pantry"?"Pantry":cuisinePantryTab==="protein"?"Protein":cuisinePantryTab==="produce"?"Produce":"Dairy")} title="Add to inventory at 0 qty, for reference" style={{padding:"3px 8px",borderRadius:6,border:"1px solid "+C.border,background:"transparent",color:C.muted,fontFamily:FM,fontSize:10,cursor:"pointer"}}>+ Inventory</button>
                            <button onClick={()=>addCuisineItemToShopping(item)} title="Add to shopping list" style={{padding:"3px 8px",borderRadius:6,border:"1px solid "+C.accent,background:"transparent",color:C.accent,fontFamily:FM,fontSize:10,cursor:"pointer",fontWeight:600}}>🛒 Shop</button>
                          </div>
                        )}
                      </div>);
                    })}
                  ...
                  <div style={{fontFamily:FM,fontSize:12,color:C.muted,padding:"10px 0"}}>Could not load items. <button onClick={()=>fetchCuisinePantry(c)} style={{background:"transparent",border:"none",color:C.accent,cursor:"pointer",textDecoration:"underline",fontFamily:FM,fontSize:12}}>Try again</button></div>
                )}
              </div>}
            </div>
          ))}
                ...
                <div style={{marginTop:10}}>
                  ...
                  {user?.id&&!cellarInvLoading&&Array.isArray(cellarInvDisplay)&&cellarInvDisplay.filter(b=>b.excludeFromCooking).length>0&&(
                    <div style={{marginTop:14}}>
                      <div style={{fontSize:10,color:C.muted,fontFamily:FM,marginBottom:8,display:"flex",alignItems:"center",gap:5}}><span style={{background:"#dc262618",border:"1px solid #dc262655",borderRadius:"50%",width:14,height:14,display:"inline-flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{color:"#dc2626",fontSize:9,fontWeight:900,lineHeight:1}}>✕</span></span> Excluded from cooking — tap to re-include</div>
                      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:8}}>
                        {cellarInvDisplay.filter(b=>b.excludeFromCooking).map((b,i)=>(
                          <div key={i} style={{background:C.card,border:"1px solid "+C.border,borderRadius:8,padding:"8px 10px",fontFamily:FM,fontSize:11,opacity:0.6}}>
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:6}}>
                              <div style={{fontWeight:600,color:C.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{b.name}</div>
                              <button onClick={()=>setCellarBottleExcluded(b.id,false)} title="Excluded — tap to re-include for Smart Kitchen suggestions" style={{background:"#dc262618",border:"1px solid #dc262655",borderRadius:"50%",width:18,height:18,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,padding:0}}><span style={{color:"#dc2626",fontSize:11,fontWeight:900,lineHeight:1}}>✕</span></button>
                            </div>
                            <div style={{color:C.muted,fontSize:10,marginTop:2,display:"flex",justifyContent:"space-between"}}>
                              <span>{b.category}</span>
                              <span>{Math.round(b.remaining_pct??100)}%</span>
...
{swapRecipeModal&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setSwapRecipeModal(null)}><div style={{background:C.card,borderRadius:12,padding:24,width:360,maxWidth:"90vw"}} onClick={e=>e.stopPropagation()}><div style={{fontFamily:FD,fontSize:18,fontWeight:700,color:C.text,marginBottom:16}}>Swap Recipe</div><button onClick={async()=>{setSwapRecipeLoading(true);const cellarInfo=await getCellarCookingBlock();const prompt="Give me ONE different recipe suggestion"+( swapRecipeRequest.trim()?" for: "+swapRecipeRequest.trim():" (different from "+swapRecipeModal.name+")")+". Inventory: "+inventory.map(i=>i.name).filter(Boolean).join(", ")+cellarInfo.block+" CRITICAL: if the requested name specifies a cooking method/technique (sous vide, air fryer, slow cooker, grilled, smoked, etc.), the instructions must actually use that exact method with correct temps/times — never keep a technique in the name while writing different, more conventional instructions. Return ONLY valid JSON: {name,description,time,difficulty,servings:4,ingredients:[{name,qty,unit} objects — qty is a NUMBER, unit is a short string like \"cup\",\"tsp\",\"tbsp\",\"oz\",\"lb\" or \"\" for countable items],instructions:[4 short strings — CRITICAL: every step referencing a measured ingredient MUST use a {{ing:N}} placeholder instead of typing a number, where N is that ingredient's zero-based index in ingredients. Never type a literal quantity.],usesFromInventory:[],missingIngredients:[]"+cellarInfo.schemaField+"}";const res=await callClaude({system:"Recipe AI. Return ONLY valid JSON, no markdown.",prompt,maxTokens:1200});try{const raw=typeof res==="string"?res:Array.isArray(res)?res.map(r=>r.text||"").join(""):res?.content?.[0]?.text||"";const clean=raw.replace(/```json|```/g,"").trim();const parsed=JSON.parse(clean);setRecipes(prev=>prev.map(r=>r.id===swapRecipeModal.id?{...parsed,id:swapRecipeModal.id,usesFromInventory:parsed.usesFromInventory||[],missingIngredients:parsed.missingIngredients||[]}:r));setSwapRecipeModal(null);}catch(e){console.error("Swap Recipe generation/parse error:",e);showAlert("Could not parse recipe.");}setSwapRecipeLoading(false);}} style={{width:"100%",padding:"12px",borderRadius:8,background:C.accent,border:"none",color:"#000",fontFamily:FM,fontSize:14,fontWeight:700,cursor:"pointer",marginBottom:10}}>{swapRecipeLoading?"Thinking...":"✦ Surprise Me"}</button><div style={{color:C.muted,fontFamily:FM,fontSize:12,textAlign:"center",marginBottom:8}}>— or request a specific recipe —</div><input value={swapRecipeRequest} onChange={e=>setSwapRecipeRequest(e.target.value)} placeholder='e.g. "Something with chicken"' style={{width:"100%",padding:"8px 12px",borderRadius:6,border:"1px solid "+C.border,background:C.surface,color:C.text,fontFamily:FM,fontSize:13,boxSizing:"border-box",marginBottom:10}}/><button onClick={async()=>{if(!swapRecipeRequest.trim()){showAlert("Please type a recipe request first.");return;}setSwapRecipeLoading(true);const cellarInfo=await getCellarCookingBlock();const prompt="Give me ONE recipe for: "+swapRecipeRequest.trim()+". Inventory: "+inventory.map(i=>i.name).filter(Boolean).join(", ")+cellarInfo.block+" CRITICAL: if the requested name specifies a cooking method/technique (sous vide, air fryer, slow cooker, grilled, smoked, etc.), the instructions must actually use that exact method with correct temps/times — never keep a technique in the name while writing different, more conventional instructions. Return ONLY valid JSON: {name,description,time,difficulty,servings:4,ingredients:[{name,qty,unit} objects — qty is a NUMBER, unit is a short string like \"cup\",\"tsp\",\"tbsp\",\"oz\",\"lb\" or \"\" for countable items],instructions:[4 short strings — CRITICAL: every step referencing a measured ingredient MUST use a {{ing:N}} placeholder instead of typing a number, where N is that ingredient's zero-based index in ingredients. Never type a literal quantity.],usesFromInventory:[],missingIngredients:[]"+cellarInfo.schemaField+"}";const res=await callClaude({system:"Recipe AI. Return ONLY valid JSON, no markdown.",prompt,maxTokens:1200});try{const raw=typeof res==="string"?res:Array.isArray(res)?res.map(r=>r.text||"").join(""):res?.content?.[0]?.text||"";const clean=raw.replace(/```json|```/g,"").trim();const parsed=JSON.parse(clean);setRecipes(prev=>prev.map(r=>r.id===swapRecipeModal.id?{...parsed,id:swapRecipeModal.id,usesFromInventory:parsed.usesFromInventory||[],missingIngredients:parsed.missingIngredients||[]}:r));setSwapRecipeModal(null);}catch(e){console.error("Swap Recipe generation/parse error:",e);showAlert("Could not parse recipe.");}setSwapRecipeLoading(false);}} style={{width:"100%",padding:"12px",borderRadius:8,background:"transparent",border:"1px solid "+C.accent,color:C.accent,fontFamily:FM,fontSize:14,cursor:"pointer",marginBottom:8}}>Make This Recipe</button><button onClick={()=>setSwapRecipeModal(null)} style={{width:"100%",padding:"8px",borderRadius:8,background:"transparent",border:"none",color:C.muted,fontFamily:FM,fontSize:12,cursor:"pointer"}}>Cancel</button></div></div>}
        {addToPlanRecipe&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.75)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setAddToPlanRecipe(null)}><div style={{background:C.card,borderRadius:14,padding:24,width:340,maxWidth:"90vw"}} onClick={e=>e.stopPropagation()}><div style={{fontFamily:FD,fontSize:20,fontWeight:700,color:C.text,marginBottom:4}}>📅 Add to Meal Plan</div><div style={{fontFamily:FM,fontSize:13,color:C.muted,marginBottom:16}}>{"Choose a day for: "+addToPlanRecipe.name}</div><div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>{["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"].map(day=>{const existing=mealPlan.find(d=>d.day===day);const isSelected=addToPlanDay===day;return(<button key={day} onClick={()=>setAddToPlanDay(day)} style={{width:"100%",padding:"10px 14px",borderRadius:8,border:"2px solid "+(isSelected?"#2aa86e":C.border),background:isSelected?"#2aa86e22":C.surface,color:isSelected?"#2aa86e":C.text,fontFamily:FM,fontSize:13,cursor:"pointer",textAlign:"left",display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontWeight:isSelected?700:400}}>{day}</span>{existing&&<span style={{fontSize:11,color:C.muted,maxWidth:"60%",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{existing.meal}</span>}</button>);})}</div><button onClick={()=>{if(!addToPlanDay){showAlert("Please select a day first.");return;}const existing=mealPlan.find(d=>d.day===addToPlanDay);if(existing&&existing.meal){setAddToPlanConfirm({recipe:addToPlanRecipe,day:addToPlanDay,existingMeal:existing.meal});setAddToPlanRecipe(null);}else{setMealPlan(prev=>prev.map(d=>d.day===addToPlanDay?{...d,meal:addToPlanRecipe.name,description:addToPlanRecipe.description||"",ingredients:addToPlanRecipe.usesFromInventory||[],shoppingNeeded:addToPlanRecipe.missingIngredients||[],proteinUsed:addToPlanRecipe.proteinUsed||"",quickMeal:addToPlanRecipe.quickMeal||false}:d));setAddToPlanRecipe(null);setAddToPlanDay("");setTab("mealplan");}}} style={{width:"100%",padding:"12px",borderRadius:8,background:"#2aa86e",border:"none",color:"#fff",fontFamily:FM,fontSize:14,fontWeight:700,cursor:"pointer",marginBottom:8}}>📅 Add to Plan</button><button onClick={()=>{setAddToPlanRecipe(null);setAddToPlanDay("");}} style={{width:"100%",padding:"8px",borderRadius:8,background:"transparent",border:"none",color:C.muted,fontFamily:FM,fontSize:12,cursor:"pointer"}}>Cancel</button></div></div>}
                    ...
                    <div style={{display:"flex",gap:8}}>
                      ...
                      {!isViewer&&<button onClick={e=>{e.stopPropagation();
                        if(!familyRecipes.find(r=>r.name===name)){
                          ...
                          showAlert(name+" moved to Family Recipes!");
                        } else { showAlert(name+" is already in Family Recipes."); }
...
{/* == MEAL PLAN == */}
        {!loading&&tab==="mealplan"&&(
          <div>
            ...
            {can.medicalCompliance&&user?.id&&(<NutritionDashboard familyProfiles={familyProfiles} user={user} supabase={supabase} seniorMode={seniorMode} C={C} FM={FM} FD={FD} refreshKey={dashRefreshKey} recallAlerts={recallAlerts}/>)}<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18,flexWrap:"wrap",gap:10}}>
                  ...
                  {mealPlan.map((day,i)=>(
                    <div key={i} style={{background:C.card,border:"1px solid "+(day.quickMeal?"#f59e0b":C.border),borderRadius:seniorMode?16:12,padding:seniorMode?18:12}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        {day.isLeftover&&<span style={{fontSize:10,background:"#f97316"+"22",color:"#f97316",padding:"2px 8px",borderRadius:4,fontFamily:FM,display:"inline-block",marginBottom:4,fontWeight:600}}>🥡 LEFTOVER — reheat, nothing to buy</span>}
                      ...
                      <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                        ...
                        <button onClick={()=>setSwapDayModal(i)} style={{background:"transparent",border:"1px solid "+C.border,borderRadius:seniorMode?10:4,color:C.muted,fontFamily:FM,fontSize:seniorMode?18:11,padding:seniorMode?"12px 20px":"8px 14px",cursor:"pointer",flexShrink:0}} disabled={isViewer}>🔀 Swap Day</button>
...
<div style={{marginBottom:14}}>
  ...
  {Array.isArray(pairDrinkCellar)&&pairDrinkCellar.length>0&&(
    <div style={{background:C.surface,border:"1px solid #7c3aed44",borderRadius:8,padding:"10px 12px",maxHeight:140,overflowY:"auto"}}>
      ...
      {pairDrinkCellar.map((b,i)=>(
        <div key={i} style={{fontFamily:FM,fontSize:11,color:C.text,padding:"3px 0",borderBottom:i<pairDrinkCellar.length-1?"1px solid "+C.border:"none",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
          <span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{b.name}</span>
          <span style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
            <span style={{color:C.muted,fontSize:10}}>{b.category}</span>
            <button onClick={async()=>{
              ...
              try{
                ...
                if(!data?.sc_cloud_data) return;
                const parsed=typeof data.sc_cloud_data==="string"?JSON.parse(data.sc_cloud_data):data.sc_cloud_data;
                const updatedCellar=(parsed.cellar||[]).map(cb=>cb.id===b.id?{...cb,excludeFromCooking:!cb.excludeFromCooking}:cb);
                await supabase.from("profiles").update({sc_cloud_data:{...parsed,cellar:updatedCellar}}).eq("id",user.id);
                setPairDrinkCellar(prev=>Array.isArray(prev)?prev.map(pb=>pb.id===b.id?{...pb,excludeFromCooking:!pb.excludeFromCooking}:pb):prev);
              }catch(e){console.error("Toggle exclude from cooking error:",e);}
            }} title={b.excludeFromCooking?"Excluded from cooking suggestions — tap to allow":"OK to cook with — tap to exclude"} style={{background:"transparent",border:"none",cursor:"pointer",fontSize:12,padding:0,lineHeight:1}}>{b.excludeFromCooking?"🚫":"🍳"}</button>
          </span>
        </div>
      ))}
    </div>
  ...
  :"";
const pref=[..."Wine","Beer","Spirits","Non-Alcoholic"].find(p=>document.getElementById("pref-"+p)?.style.background.includes("7c3aed22"))||"";setPairDrinkLoading(true);setPairDrinkResult(null);setPairDrinkSaleAddStatus(null);try{const alcoholSaleAds=await fetchAlcoholSaleAds(user?.id);const saleListForPrompt=alcoholSaleAds.length>0?alcoholSaleAds.map(a=>a.name+" | $"+a.price.toFixed(2)+" | "+(a.store||"unknown store")).join("\n"):null;const resp=await callClaude({system:"You are a knowledgeable sommelier and beverage pairing expert. Return ONLY valid JSON, no markdown, no other text. The user will give you a meal name, optional cellar inventory, an optional preference, and an optional list of alcohol currently on sale at their preferred stores (each line: name | price | store). Return this exact shape: {\"cellarPick\":\"2-3 sentences recommending the best pairing from their cellar inventory, leading with the bottle name, or null if no cellar inventory was given or nothing fits\",\"saleItem\":{\"name\":\"exact name from the sale list\",\"price\":number,\"store\":\"exact store from the sale list\"} or null if nothing on the sale list genuinely pairs well with this meal -- never invent a sale item that wasn\'t in the provided list, and never fill store with anything other than the exact store text given for that item,\"saleReasoning\":\"2-3 sentences on why that specific sale item pairs well with this meal, only present if saleItem is non-null\",\"lookingAhead\":\"2-3 sentences on what styles would complement this type of cooking and be worth having on hand, or null if not relevant\"}",prompt:"Meal: "+pairDrinkMeal.meal+(pairDrinkMeal.proteinUsed?". Protein: "+pairDrinkMeal.proteinUsed:"")+(pairDrinkMeal.sideUsed?". Side: "+pairDrinkMeal.sideUsed:"")+(inv?"\n\nCellar inventory:\n"+inv:"\n\nNo cellar inventory provided.")+(pref?"\n\nPreference: "+pref:"")+(saleListForPrompt?"\n\nCurrently on sale at preferred stores (name | price | store):\n"+saleListForPrompt:"\n\nNo sale data available."),maxTokens:600});const rawText=typeof resp==="string"?resp:resp?.content?.[0]?.text||"";try{const cleanJson=rawText.replace(/```json|```/g,"").trim();const s=cleanJson.indexOf("{"),e=cleanJson.lastIndexOf("}");const parsed=JSON.parse(cleanJson.slice(s,e+1));const stripMd=(t)=>t?String(t).replace(/[*][*]([^*]+)[*][*]/g,"$1"):null;setPairDrinkResult({cellarPick:stripMd(parsed.cellarPick),saleItem:parsed.saleItem&&parsed.saleItem.name?parsed.saleItem:null,saleReasoning:stripMd(parsed.saleReasoning),lookingAhead:stripMd(parsed.lookingAhead)});}catch(parseErr){console.error("Pairing JSON parse error:",parseErr,rawText);setPairDrinkResult({cellarPick:rawText.replace(/[*][*]([^*]+)[*][*]/g,"$1")||"No recommendation returned.",saleItem:null,saleReasoning:null,lookingAhead:null});}}catch(e){setPairDrinkResult({cellarPick:"Unable to get recommendation. Please try again.",saleItem:null,saleReasoning:null,lookingAhead:null});}finally{setPairDrinkLoading(false);}}} disabled={pairDrinkLoading} style={{width:"100%",background:pairDrinkLoading?"#7c3aed44":"#7c3aed",border:"none",borderRadius:10,color:"#fff",fontFamily:FM,fontWeight:700,fontSize:seniorMode?16:13,padding:seniorMode?"14px":"11px 16px",cursor:pairDrinkLoading?"not-allowed":"pointer",marginBottom:14}}>{pairDrinkLoading?"⏳ Finding your pairing...":"🍷 Get Pairing Recommendation"}</button>{pairDrinkResult&&<div>{pairDrinkResult.cellarPick&&<div style={{background:"#7c3aed11",border:"1px solid #7c3aed33",borderRadius:10,padding:16,fontFamily:FM,fontSize:seniorMode?15:13,color:C.text,lineHeight:1.6,whiteSpace:"pre-wrap",marginBottom:10}}><div style={{fontFamily:FM,fontSize:10,fontWeight:700,color:"#7c3aed",letterSpacing:0.6,marginBottom:6}}>FROM YOUR CELLAR</div>{pairDrinkResult.cellarPick}</div>}{pairDrinkResult.saleItem&&<div style={{background:"#14b8a611",border:"1px solid #14b8a644",borderRadius:10,padding:16,marginBottom:10}}><div style={{fontFamily:FM,fontSize:10,fontWeight:700,color:"#14b8a6",letterSpacing:0.6,marginBottom:6}}>WORTH BUYING ON SALE</div><div style={{fontFamily:FM,fontSize:seniorMode?16:14,fontWeight:700,color:C.text,marginBottom:2}}>{pairDrinkResult.saleItem.name}</div><div style={{fontFamily:FM,fontSize:12,color:"#14b8a6",marginBottom:8}}>🛒 {pairDrinkResult.saleItem.store||"Store not specified"} — ${Number(pairDrinkResult.saleItem.price).toFixed(2)}</div>{pairDrinkResult.saleReasoning&&<div style={{fontFamily:FM,fontSize:seniorMode?15:13,color:C.text,lineHeight:1.6,marginBottom:10}}>{pairDrinkResult.saleReasoning}</div>}<button onClick={()=>{const item=pairDrinkResult.saleItem;const alreadyOn=shopping.some(s=>(s.name||"").toLowerCase()===item.name.toLowerCase());if(alreadyOn){setPairDrinkSaleAddStatus("already");return;}setShopping(prev=>[...prev,{name:item.name,qty:1,unit:"bottle",category:"Smart Cellar",checked:false,source:"Pair a Drink",assignedStore:item.store||null,assignedPrice:item.price??null}]);setPairDrinkSaleAddStatus("added");}} style={{width:"100%",padding:seniorMode?"12px":"9px 14px",borderRadius:8,border:"none",fontFamily:FM,fontWeight:700,fontSize:seniorMode?15:12,cursor:"pointer",background:"#14b8a6",color:"#fff"}}>{pairDrinkSaleAddStatus==="added"?"✓ Added to Shopping List!":pairDrinkSaleAddStatus==="already"?"Already on your list":"🛒 Add to Shopping List"}</button></div>}{pairDrinkResult.lookingAhead&&<div style={{background:C.surface,border:"1px dashed #7c3aed44",borderRadius:10,padding:14,fontFamily:FM,fontSize:seniorMode?14:12,color:C.muted,lineHeight:1.6,marginBottom:14}}><div style={{fontFamily:FM,fontSize:10,fontWeight:700,color:"#7c3aed",letterSpacing:0.6,marginBottom:6}}>LOOKING AHEAD</div>{pairDrinkResult.lookingAhead}</div>}{Array.isArray(pairDrinkCellar)&&pairDrinkCellar.length>0&&(<div style={{background:C.surface,border:"1px solid #7c3aed33",borderRadius:10,padding:"12px 14px"}}><div style={{fontFamily:FM,fontSize:11,fontWeight:700,color:"#7c3aed",marginBottom:8}}>MARK AS USED</div><div style={{fontFamily:FM,fontSize:11,color:C.muted,marginBottom:10}}>Which bottle did you open? We'll update your Smart Cellar inventory.</div><div style={{display:"flex",flexDirection:"column",gap:6,maxHeight:120,overflowY:"auto",marginBottom:10}}>{pairDrinkCellar.map((b,i)=>(<button key={i} onClick={()=>setPairDrinkMarkedBottle(pairDrinkMarkedBottle?.id===b.id?null:b)} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 10px",borderRadius:8,cursor:"pointer",fontFamily:FM,fontSize:12,border:"1px solid "+(pairDrinkMarkedBottle?.id===b.id?"#7c3aed":"#7c3aed33"),background:pairDrinkMarkedBottle?.id===b.id?"#7c3aed22":"transparent",color:C.text,textAlign:"left"}}><span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flex:1}}>{b.name}</span><span style={{color:C.muted,fontSize:10,flexShrink:0,marginLeft:8}}>{Math.round(b.remaining_pct??100)}%</span></button>))}</div><button disabled={!pairDrinkMarkedBottle||pairDrinkMarkStatus==="saving"} onClick={async()=>{if(!pairDrinkMarkedBottle||!user)return;setPairDrinkMarkStatus("saving");try{const {data}=await supabase.from("profiles").select("sc_cloud_data").eq("id",user.id).single();if(data?.sc_cloud_data){const parsed=typeof data.sc_cloud_data==="string"?JSON.parse(data.sc_cloud_data):data.sc_cloud_data;const cat=(pairDrinkMarkedBottle.category||"").toLowerCase();const bottleMl=pairDrinkMarkedBottle.size_ml||750;const pourMl=cat.includes("wine")||cat.includes("sparkling")||cat.includes("ros")?148:cat.includes("beer")||cat.includes("cider")||cat.includes("seltzer")?355:cat.includes("whiskey")||cat.includes("bourbon")||cat.includes("scotch")||cat.includes("rye")||cat.includes("vodka")||cat.includes("gin")||cat.includes("rum")||cat.includes("tequila")||cat.includes("brandy")||cat.includes("spirit")?44:74;const POUR_PCT=Math.round((pourMl/bottleMl)*100);const prevPct=pairDrinkMarkedBottle.remaining_pct??100;const updatedCellar=(parsed.cellar||[]).map(b=>{if(b.id===pairDrinkMarkedBottle.id){const newPct=Math.max(0,(b.remaining_pct??100)-POUR_PCT);return {...b,remaining_pct:newPct};}return b;});await supabase.from("profiles").update({sc_cloud_data:{...parsed,cellar:updatedCellar}}).eq("id",user.id);const updatedBottle=updatedCellar.find(b=>b.id===pairDrinkMarkedBottle.id);const newPct=updatedBottle?.remaining_pct??0;if(prevPct>=75&&newPct<75){const alreadyOn=shopping.some(e=>(e.name||"").toLowerCase()===pairDrinkMarkedBottle.name.toLowerCase());if(!alreadyOn){setShopping(prev=>[...prev,{name:pairDrinkMarkedBottle.name,qty:1,unit:"bottle",category:"Smart Cellar",checked:false,source:"Smart Cellar — running low"}]);setPairDrinkMarkStatus("listed");}else{setPairDrinkMarkStatus("done");}}else{setPairDrinkMarkStatus("done");}setPairDrinkCellar(updatedCellar.filter(b=>(b.remaining_pct??100)>5));}}catch(e){console.error("Mark as used error:",e);setPairDrinkMarkStatus("error");}}} style={{width:"100%",padding:seniorMode?"12px":"9px 14px",borderRadius:8,border:"none",fontFamily:FM,fontWeight:700,fontSize:seniorMode?15:12,cursor:pairDrinkMarkedBottle&&pairDrinkMarkStatus!=="saving"?"pointer":"not-allowed",background:pairDrinkMarkedBottle?"#7c3aed":"#7c3aed44",color:"#fff",opacity:pairDrinkMarkedBottle?1:0.5}}>{pairDrinkMarkStatus==="saving"?"⏳ Updating cellar…":pairDrinkMarkStatus==="done"?"✓ Cellar Updated!":pairDrinkMarkStatus==="listed"?"✓ Updated + Added to Shopping List!":pairDrinkMarkStatus==="error"?"✕ Could not save — try again":"✓ Mark as Used"}</button>{pairDrinkMarkStatus==="listed"&&<div style={{fontFamily:FM,fontSize:11,color:"#7c3aed",marginTop:8,textAlign:"center"}}>🛒 {pairDrinkMarkedBottle?.name} added to your shopping list.</div>}{pairDrinkMarkStatus==="done"&&<div style={{fontFamily:FM,fontSize:11,color:C.muted,marginTop:8,textAlign:"center"}}>Smart Cellar updated. Changes sync next time you open Smart Cellar.</div>}</div>)}</div>}</div></div>}
        ...
        </div><div style={{marginBottom:16}}><button onClick={async()=>{setChangeMealLoading(true);try{const h=JSON.parse(localStorage.getItem("sk_changeMealHistory")||"[]");const d=mealPlan[changeMealModal];if(d){h.push({meal:d.meal,protein:d.proteinUsed||null,day:d.day,ts:Date.now()});localStorage.setItem("sk_changeMealHistory",JSON.stringify(h.slice(-100)));}}catch{} const day=mealPlan[changeMealModal];const activeSaleItems=saleItems.filter(i=>i.effectiveWeek!=="preview");const saleContext=activeSaleItems.length>0?` This week's active sale items: ${activeSaleItems.map(i=>i.name+(i.salePrice?" ("+i.salePrice+")":"")).join(", ")}. If any of these fit well for a dinner, prioritize using them to keep savings consistent with the rest of the week's plan — don't just fall back to a plain inventory-only suggestion when a good sale option is available.`:"";const leftoverList=inventory.filter(i=>i.isLeftover&&i.qty>0).map(i=>i.name+" ("+i.qty+" servings, use by "+i.useBy+")");const leftoverContext=leftoverList.length>0?` LEFTOVER MEALS AVAILABLE IN THE FRIDGE: ${leftoverList.join(", ")}. If one of these is a good fit for a quick/busy night, suggest reheating it instead of a from-scratch recipe — set isLeftover:true, meal to the exact leftover name, ingredients:[], needToBuy:[], proteinUsed:"", and sauteBagsUsed:0 (nothing to buy or cook, it's already made).`:"";const prompt=`${buildOccasionContext(occasionState)}Suggest a different dinner meal for ${day.day}. Current meal was: ${day.meal}. INVENTORY (items already owned — do NOT put these in needToBuy): ${inventory.filter(i=>(parseFloat(i.qty)??1)>0).map(i=>String(i.name||"")).filter(Boolean).join(", ")}.${saleContext}${leftoverContext} STRICT RULE: needToBuy must contain ONLY ingredients required for this meal that are NOT in the inventory list above. If an ingredient appears in inventory, it must NOT appear in needToBuy. Cross-check every needToBuy item against inventory before returning. Return JSON: {meal,ingredients:[],needToBuy:[],proteinUsed:"",sauteBagsUsed:0,quickMeal:false,isLeftover:false}.`;const res=await callClaude({system:"Meal planning AI. Return ONLY valid JSON, no markdown.",prompt,maxTokens:600});try{console.log("changeMeal res:",JSON.stringify(res));const resText=typeof res==="string"?res:Array.isArray(res)?res.map(r=>r.text||"").join(""):res?.content?.[0]?.text||res?.[0]?.text||"";const raw=resText.replace(/```json|```/g,"").trim();console.log("raw:",raw);const s=raw.indexOf("{"),e=raw.lastIndexOf("}");const parsed=JSON.parse(raw.slice(s,e+1));setMealPlan(p=>p.map((d,i)=>i===changeMealModal?{...d,...parsed,needToBuy:parsed.needToBuy||[],shoppingNeeded:(parsed.needToBuy||[]).map(n=>typeof n==="string"?{qty:1,unit:"",name:n}:n),ingredients:parsed.ingredients||[]}:d));setExpandedIngDay(null);setChangeMealModal(null);}catch(err){console.error("Parse error:",err);showAlert("Could not parse meal suggestion");}setChangeMealLoading(false);}} style={{width:"100%",padding:seniorMode?"14px":"10px",background:C.accent,border:"none",borderRadius:8,color:"#000",fontFamily:FM,fontSize:seniorMode?16:13,fontWeight:600,cursor:"pointer",marginBottom:10}}>✨ {changeMealLoading?"Thinking...":"Surprise Me"}</button><div style={{fontFamily:FM,fontSize:12,color:C.muted,marginBottom:8}}>— or request a specific meal —</div><input style={{width:"100%",padding:"8px",background:C.surface,border:"1px solid "+C.border,borderRadius:6,color:C.text,fontFamily:FM,fontSize:13,boxSizing:"border-box",marginBottom:10}} placeholder='e.g. "Goulash"' value={changeMealRequest} onChange={e=>setChangeMealRequest(e.target.value)} onClick={e=>e.stopPropagation()} onFocus={e=>e.stopPropagation()} /><button onClick={async()=>{if(!changeMealRequest.trim())return;setChangeMealLoading(true);try{const h=JSON.parse(localStorage.getItem("sk_changeMealHistory")||"[]");const d=mealPlan[changeMealModal];if(d){h.push({meal:d.meal,protein:d.proteinUsed||null,day:d.day,ts:Date.now()});localStorage.setItem("sk_changeMealHistory",JSON.stringify(h.slice(-100)));}}catch{} const day=mealPlan[changeMealModal];const leftoverList2=inventory.filter(i=>i.isLeftover&&i.qty>0).map(i=>i.name+" ("+i.qty+" servings, use by "+i.useBy+")");const leftoverContext2=leftoverList2.length>0?` LEFTOVER MEALS AVAILABLE IN THE FRIDGE: ${leftoverList2.join(", ")}. If the requested meal matches one of these leftovers (even approximately, e.g. "mac and cheese" matching "Baked Mac and Cheese"), it's already made -- set isLeftover:true, meal to the exact leftover's name, ingredients:[], needToBuy:[], proteinUsed:"", and sauteBagsUsed:0 (nothing to buy or cook, just reheat). Do not write a from-scratch recipe for something that's already sitting in the fridge as a leftover.`:"";const prompt=`Create a dinner meal for ${day.day} using "${changeMealRequest}". INVENTORY (items already owned — do NOT put these in needToBuy): ${inventory.filter(i=>(parseFloat(i.qty)??1)>0).map(i=>String(i.name||"")).filter(Boolean).join(", ")}.${leftoverContext2} STRICT RULE: needToBuy must contain ONLY ingredients required for this meal that are NOT in the inventory list above. If an ingredient appears in inventory, it must NOT appear in needToBuy. Cross-check every needToBuy item against inventory before returning. Return JSON: {meal,ingredients:[],needToBuy:[],proteinUsed:"",sauteBagsUsed:0,quickMeal:false,isLeftover:false}.`;const res=await callClaude({system:"Meal planning AI. Return ONLY valid JSON, no markdown.",prompt,maxTokens:600});try{const resText3=typeof res==="string"?res:Array.isArray(res)?res.map(r=>r.text||"").join(""):res?.content?.[0]?.text||res?.[0]?.text||"";const raw=resText3;const s=raw.indexOf("{"),e=raw.lastIndexOf("}");const parsed=JSON.parse(raw.slice(s,e+1));setMealPlan(p=>p.map((d,i)=>i===changeMealModal?{...d,...parsed,needToBuy:parsed.needToBuy||[],shoppingNeeded:(parsed.needToBuy||[]).map(n=>typeof n==="string"?{qty:1,unit:"",name:n}:n),ingredients:parsed.ingredients||[]}:d));setExpandedIngDay(null);setChangeMealModal(null);}catch(e){showAlert("Could not parse meal suggestion");}setChangeMealLoading(false);}} style={{width:"100%",padding:"10px",background:"transparent",border:"1px solid "+C.accent,borderRadius:8,color:C.accent,fontFamily:FM,fontSize:13,cursor:"pointer"}}>🍽 {changeMealLoading?"Thinking...":"Make This Meal"}</button></div><button onClick={()=>setChangeMealModal(null)} style={{width:"100%",padding:"8px",background:"transparent",border:"none",color:C.muted,fontFamily:FM,fontSize:12,cursor:"pointer"}}>Cancel</button></div></div>}
        {swapDayModal!==null&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setSwapDayModal(null)}><div style={{background:C.card,borderRadius:12,padding:24,width:360,maxWidth:"90vw"}} onClick={e=>e.stopPropagation()}>
          <div style={{fontFamily:FD,fontSize:18,fontWeight:700,color:C.text,marginBottom:6}}>🔀 Swap {mealPlan[swapDayModal]?.day}'s Meal</div>
          <div style={{fontFamily:FM,fontSize:12,color:C.muted,marginBottom:16}}>Currently: {mealPlan[swapDayModal]?.meal}. Pick a day to trade with — nothing gets regenerated, the two meals just switch places.</div>
          {mealPlan.map((d,j)=>j!==swapDayModal&&(
            <button key={j} onClick={()=>swapDays(swapDayModal,j)} style={{width:"100%",textAlign:"left",padding:"10px 12px",marginBottom:8,background:C.surface,border:"1px solid "+C.border,borderRadius:8,color:C.text,fontFamily:FM,fontSize:13,cursor:"pointer"}}>
              <strong>{d.day}</strong> — {d.meal}
            </button>
          ))}
          <button onClick={()=>setSwapDayModal(null)} style={{width:"100%",padding:"8px",background:"transparent",border:"none",color:C.muted,fontFamily:FM,fontSize:12,cursor:"pointer",marginTop:4}}>Cancel</button>
        </div></div>}
{/* == SHOPPING == */}
...
{budgetAmount&&shopping.length>0&&(()=>{
  const priced=shopping.map(s=>{
    ...
    return {...s,estPrice:match?(match.avgUnitPrice||match.lastPrice||null):null};
  });
  const estimated=priced.filter(p=>p.estPrice);
  const estTotal=estimated.reduce((sum,p)=>sum+p.estPrice,0);
  const projected=periodSpend+estTotal;
  const over=projected>parseFloat(budgetAmount);
  return (
    <div style={{background:over?"#dc262611":"#22c55e11",border:"1px solid "+(over?"#dc262644":"#22c55e44"),borderRadius:10,padding:"10px 14px",marginBottom:14}}>
      <div style={{fontFamily:FM,fontSize:12,fontWeight:700,color:over?"#dc2626":"#16a34a"}}>{over?"⚠️ ":"💰 "}{budgetPeriod} Budget: ${parseFloat(budgetAmount).toFixed(2)}</div>
      <div style={{fontFamily:FM,fontSize:11,color:C.muted,marginTop:2}}>Spent so far: ${periodSpend.toFixed(2)} · This list: ~${estTotal.toFixed(2)} ({estimated.length}/{shopping.length} items priced) · Projected: ${projected.toFixed(2)}{over?" — over budget":""}</div>
    </div>
  );
})()}
{user&&(
...
{user&&(
  ...
  onClick={async()=>{if(cellarPullStatus==="saving")return;setCellarPullStatus("saving");try{const {data}=await supabase.from("profiles").select("sc_cloud_data").eq("id",user.id).single();if(data?.sc_cloud_data){const parsed=typeof data.sc_cloud_data==="string"?JSON.parse(data.sc_cloud_data):data.sc_cloud_data;const cellarList=parsed.shoppingList||[];if(cellarList.length===0){setCellarPullStatus("empty");setTimeout(()=>setCellarPullStatus(null),3000);return;}const toAdd=cellarList.filter(ci=>!shopping.some(si=>(si.name||"").toLowerCase()===(ci.name||"").toLowerCase()));if(toAdd.length>0){setShopping(p=>[...p,...toAdd.map(ci=>({name:ci.name,qty:1,unit:"bottle",category:"Smart Cellar",checked:false,source:"Smart Cellar Advisor"}))]);setCellarPullStatus("done");}else{setCellarPullStatus("already");}}else{setCellarPullStatus("empty");}}catch(e){console.error("Cellar pull error:",e);setCellarPullStatus("error");}setTimeout(()=>setCellarPullStatus(null),3000);}}>
    ...
    <span>{cellarPullStatus==="saving"?"⏳ Loading…":cellarPullStatus==="done"?"✓ Cellar Items Added!":cellarPullStatus==="empty"?"🍾 No items in Smart Cellar Buy List":cellarPullStatus==="already"?"✓ Already on List":cellarPullStatus==="error"?"✕ Could not load":"🍾 Add from Smart Cellar Buy List"}</span>
                ...
                return(
                  <div key={profile.id} style={{background:C.card,border:"1px solid "+isEditing?C.accent:C.border,borderRadius:12,padding:14,transition:"border-color 0.15s"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:isEditing?12:0}}>
                      ...
                      <button onClick={()=>{showConfirm("Remove "+(profile.name||"this member")+" from your household? This cannot be undone.",()=>{setFamilyProfiles(prev=>prev.filter(p=>p.id!==profile.id));setFamilySize(prev=>Math.max(1,prev-1));if(editingProfile===profile.id)setEditingProfile(null);});}} style={{...bBtn("ghost"),padding:"5px 10px",fontSize:11,border:"1px solid #dc2626",color:"#dc2626"}}>✕ Remove</button>
                    </div>
                    {isEditing&&(
                      <div style={{display:"flex",flexDirection:"column",gap:10}}>
                        <div><Label>NAME</Label><input style={bInp} placeholder={"Family member "+(idx+1)} value={profile.name} onChange={e=>setFamilyProfiles(p=>p.map(pr=>pr.id===profile.id?{...pr,name:e.target.value}:pr))}/></div><div style={{marginTop:10}}><Label>DATE OF BIRTH <span style={{fontWeight:400,color:C.muted,fontSize:9}}>(optional)</span></Label><input type="date" style={{...bInp,colorScheme:darkMode?"dark":"light"}} value={profile.dob||""} max={new Date().toISOString().split("T")[0]} onChange={e=>{const v=e.target.value;setFamilyProfiles(p=>p.map(pr=>{if(pr.id!==profile.id) return pr;const computedAge=v?Math.floor((new Date()-new Date(v+"T12:00:00"))/(1000*60*60*24*365.25)):pr.age;return {...pr,dob:v,age:computedAge};}));}}/>{profile.dob&&(()=>{const age=Math.floor((new Date()-new Date(profile.dob+"T12:00:00"))/(1000*60*60*24*365.25));const today=new Date();const bday=new Date(profile.dob+"T12:00:00");const nextBday=new Date(today.getFullYear(),bday.getMonth(),bday.getDate());if(nextBday<today) nextBday.setFullYear(today.getFullYear()+1);const daysUntil=Math.ceil((nextBday-today)/(1000*60*60*24));return <div style={{fontFamily:FM,fontSize:11,color:C.muted,marginTop:4,display:"flex",gap:10,flexWrap:"wrap"}}><span style={{color:C.text,fontWeight:600}}>Age {age}</span>{daysUntil<=30&&<span style={{color:C.accent,fontWeight:600}}>🎂 Birthday in {daysUntil} day{daysUntil===1?"":"s"}!</span>}{daysUntil>30&&daysUntil<=365&&<span>🎂 Birthday in {daysUntil} days</span>}</div>;})()}</div>
                        <div style={{marginTop:10,display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                          <div><Label>HEIGHT <span style={{fontWeight:400,color:C.muted,fontSize:9}}>(optional)</span></Label><input style={bInp} placeholder="e.g. 5ft 8in" value={profile.heightStr||""} onChange={e=>setFamilyProfiles(p=>p.map(pr=>pr.id===profile.id?{...pr,heightStr:e.target.value}:pr))}/></div>
                ...
                {showTempForm&&(
                  <div style={{background:"#1a0a2e",borderRadius:10,padding:14,marginBottom:12,border:"1px solid #a78bfa44"}}>
                    ...
                    <button style={{...bBtn("primary"),width:"100%",background:"#7c3aed"}} onClick={()=>{
                      if(!newTemp.name||!newTemp.reason){showAlert("Please enter a name and reason.");return;}
              ...
              <div>
                ...
                <div style={{display:"flex",gap:8}}>
                  ...
                  <button style={{...bBtn("teal"),flex:2}} onClick={()=>{
                    ...
                    if(bags===0){showAlert("Not enough veg for a 2-cup bag.");return;}
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,padding:16}} onClick={()=>setInstacartCopyCue(null)}>
          <div style={{background:C.card,border:"1px solid #f5710055",borderRadius:16,padding:26,maxWidth:420,width:"100%"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{fontSize:13,color:C.text,lineHeight:1.6,marginBottom:14,textAlign:"center"}}>
              {instacartCopyCue.isOtherStore
                ? (instacartCopyCue.copied
                    ? "We opened Instacart's store picker in a new tab — choose whichever store is available at your address, then look for Cart Assistant in that store's sidebar and paste (Ctrl/Cmd+V) into the chat box."
                    : "We opened Instacart's store picker in a new tab, but couldn't copy automatically — choose your store, then select and copy the list below and paste it into Cart Assistant.")
                : (instacartCopyCue.copied
                    ? "We opened Instacart in a new tab. Click Cart Assistant in the sidebar, then paste (Ctrl/Cmd+V) into the chat box to build your cart."
      ...
      {/* == APP ALERT (replaces native alert()) == */}
      {appAlert&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:3000,padding:16}} onClick={()=>setAppAlert(null)}>
          <div style={{background:C.card,border:"1px solid "+C.border,borderRadius:16,padding:26,maxWidth:380,width:"100%",textAlign:"center"}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:seniorMode?17:13,color:C.text,lineHeight:1.6,marginBottom:18,whiteSpace:"pre-wrap"}}>{appAlert}</div>
            <button style={{...bBtn("primary"),width:"100%"}} onClick={()=>setAppAlert(null)}>OK</button>
          </div>
        </div>
      )}
      {/* == APP CONFIRM (replaces native window.confirm()) == */}
      {appConfirm&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:3000,padding:16}} onClick={()=>setAppConfirm(null)}>
          <div style={{background:C.card,border:"1px solid "+C.border,borderRadius:16,padding:26,maxWidth:380,width:"100%",textAlign:"center"}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:seniorMode?17:13,color:C.text,lineHeight:1.6,marginBottom:18,whiteSpace:"pre-wrap"}}>{appConfirm.message}</div>
            <div style={{display:"flex",gap:8}}>
              <button style={{...bBtn("ghost"),flex:1}} onClick={()=>setAppConfirm(null)}>Cancel</button>
              <button style={{...bBtn("primary"),flex:1}} onClick={()=>{const fn=appConfirm.onConfirm;setAppConfirm(null);if(fn)fn();}}>Confirm</button>
            </div>
          </div>
        </div>
      )}
      {/* == FOOD RECALL DETAIL MODAL == */}
      {activeRecallDetail&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:3000,padding:16}} onClick={()=>setActiveRecallDetail(null)}>
          <div style={{background:C.card,border:"1px solid "+C.border,borderRadius:16,padding:22,maxWidth:520,width:"100%",maxHeight:"80vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
            ...
            {activeRecallDetail.map(a=>{
              ...
              const isPending=a.severity==="pending";
              const tierColor=isCritical?"#ef4444":isPending?"#8b5cf6":"#d97706";
              const tierBg=isCritical?"#4a0e0e":isPending?"#2e1a4a":"#3a2a0e";
              const tierText=isCritical?"#fecaca":isPending?"#ddd6fe":"#fde68a";
              return (
                <div key={a.id} style={{background:C.surface,border:"1px solid "+tierColor,borderRadius:10,padding:14,marginBottom:12}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                    <span style={{fontSize:11,fontWeight:700,padding:"3px 10px",borderRadius:20,background:tierBg,color:tierText}}>{r.classification||(isPending?"🆕 Recall Announced — FDA Classification Pending":"Class I")}</span>
                  ...
                  {r.recall_initiation_date&&<div style={{fontSize:11,color:C.muted,marginBottom:8}}>Recall date: {r.recall_initiation_date}</div>}
                  {isPending&&<div style={{fontSize:11,color:"#c4b5fd",marginBottom:8,fontStyle:"italic"}}>This recall was just announced by FDA and hasn't been formally classified for severity yet — that typically takes days to weeks. Treat it as worth checking regardless.</div>}
                  <button onClick={()=>dismissRecallAlert(a.id)} style={{...bBtn("ghost"),fontSize:11,padding:"6px 12px"}}>Dismiss</button>
                </div>
              );
            })}
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1100,padding:16}} onClick={()=>setShowPricingModal(false)}>
          <div style={{background:C.card,border:"1px solid #C8963E55",borderRadius:16,padding:24,maxWidth:420,width:"100%",maxHeight:"85vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
              ...
              <button onClick={()=>setShowPricingModal(false)} style={{background:"transparent",border:"none",fontSize:20,cursor:"pointer",color:C.muted}}>✕</button>
            </div>
            <div style={{fontSize:11,color:pricingError?"#ef4444":"#22c55e",fontFamily:FM,marginBottom:16,fontWeight:600}}>
              {pricingLoading?"Loading live pricing from Stripe…":pricingError?pricingError:"✓ Live from Stripe"+(pricingFetchedAt?" — checked "+new Date(pricingFetchedAt).toLocaleTimeString():"")}
            </div>
            {pricingError&&<button onClick={fetchCurrentPricing} style={{...bBtn("ghost"),width:"100%",marginBottom:12,border:"1px solid "+C.border}}>Try Again</button>}
            {pricingLoading&&<div style={{textAlign:"center",padding:20}}><LoadingDots/></div>}
            {!pricingLoading&&!pricingError&&livePricing&&(()=>{
              const grouped={};
              livePricing.forEach(p=>{
                const tierKey=p.key.replace(/_monthly$|_annual$/,"");
                if(!grouped[tierKey])grouped[tierKey]={};
                grouped[tierKey][p.interval]=p;
              });
              const tierLabels={solo:"Solo",couple:"Couple",family:"Family",medical_addon:"Medical+ (add-on to any tier)"};
              const order=["solo","couple","family","medical_addon"];
              return order.filter(k=>grouped[k]).map(k=>{
                const t=grouped[k];
                return (
                  <div key={k} style={{background:C.surface,borderRadius:10,padding:14,marginBottom:10}}>
                    <div style={{fontFamily:FD,fontSize:14,fontWeight:700,color:C.text,marginBottom:8}}>{tierLabels[k]||k}</div>
                    <div style={{display:"flex",gap:20}}>
                      {t.month&&<div><div style={{fontSize:19,fontWeight:700,color:"#C8963E",fontFamily:FM}}>${t.month.amount.toFixed(2)}<span style={{fontSize:11,color:C.muted,fontWeight:400}}>/mo</span></div></div>}
                      {t.year&&<div><div style={{fontSize:19,fontWeight:700,color:"#C8963E",fontFamily:FM}}>${t.year.amount.toFixed(2)}<span style={{fontSize:11,color:C.muted,fontWeight:400}}>/yr</span></div></div>}
                    </div>
                  </div>
                );
              });
            })()}
            <div style={{fontSize:10,color:C.dim,fontFamily:FM,marginTop:6,lineHeight:1.5}}>30-day free trial on every paid tier, no credit card required. Prices shown are pulled directly from Stripe the moment you open this screen — not a cached copy.</div>
          </div>
        </div>
      )}
              ...
              <div>
                ...
                {scanMode==="weeklyad"?(
                  ...
                  <div style={{display:"flex",gap:8,marginTop:12}}>
                    <button style={{...bBtn("ghost"),flex:1}} onClick={()=>{setScanStage("upload");setScanResults(null);}}>Rescan</button>
                    <button style={{...bBtn("green"),flex:2}} onClick={()=>{const chosen=scanResults.filter(i=>i.selected);setShopping(prev=>{const added=chosen.filter(c=>!prev.some(e=>e.name.toLowerCase()===c.name.toLowerCase()));return [...prev,...added.map(c=>({name:c.name,qty:1,unit:"item",category:c.category||"Other",checked:false,source:"Whiteboard List"}))];});setScanOpen(false);setScanPreview(null);setScanB64(null);setScanResults(null);setScanStage("upload");setTab("shopping");showAlert(chosen.length+" item"+(chosen.length!==1?"s":"")+" added to your shopping list!");}}>🛒 Add {scanResults.filter(i=>i.selected).length} to Shopping List</button>
                  </div>
                ):(
                  <div style={{display:"flex",gap:8,marginTop:12}}>
        ...
        <div style={{background:C.surface,border:"1px solid "+C.borderLight,borderRadius:16,padding:24,maxWidth:440,width:"100%",maxHeight:"92vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
          ...
          {canIHaveResult&&(
            <div>
              ...
              <div style={{fontSize:10,color:C.muted,fontStyle:"italic",textAlign:"center",marginBottom:14,lineHeight:1.5}}>Your physician is the final authority on your health — Smart Kitchen only assists with day-to-day implementation.</div>
          ...
          <div style={{background:C.surface,border:"1px solid "+C.borderLight,borderRadius:16,padding:22,maxWidth:500,width:"100%",maxHeight:"88vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:6,background:"#fef3c7",borderRadius:20,padding:"4px 10px",border:"1px solid #f59e0b"}}>
                ...
                <button onClick={()=>setActiveRecipeServings(s=>Math.max(1,s-1))} style={{background:"#f59e0b",border:"none",borderRadius:"50%",width:20,height:20,cursor:"pointer",fontSize:12,color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700}}>−</button>
                <span style={{fontFamily:FM,fontSize:14,color:"#92400e",fontWeight:700,minWidth:16,textAlign:"center"}}>{activeRecipeServings}</span>
                <button onClick={()=>setActiveRecipeServings(s=>s+1)} style={{background:"#f59e0b",border:"none",borderRadius:"50%",width:20,height:20,cursor:"pointer",fontSize:12,color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700}}>+</button>
              </div>
            </div>
            ...
            {(()=>{
              const scale=activeRecipeServings/(activeRecipe.servings||4);
              return (activeRecipe.instructions||[]).map((step,i)=>(
                <div key={i} style={{display:"flex",gap:10,marginBottom:12,alignItems:"flex-start"}}>
                  ...
                  <div style={{fontSize:13,lineHeight:1.7,color:C.text}}>{renderStepText(step,activeRecipe.ingredients,scale)}</div>
                </div>
              ));
            })()}
          ...
          <div style={{background:C.card,border:"1px solid "+C.border,borderRadius:18,padding:24,maxWidth:460,width:"100%",maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
            ...
            {shareResult&&(
              <div style={{textAlign:"center"}}>
                ...
                <div style={{display:"flex",gap:8,marginBottom:16}}>
                  <button onClick={()=>{navigator.clipboard?.writeText(shareResult.code);showAlert("Code copied!");}}
                    style={{...bBtn("ghost"),flex:1,padding:"10px",fontSize:13}}>📋 Copy Code</button>
                  <button onClick={()=>{navigator.clipboard?.writeText(shareResult.url);showAlert("Link copied!");}}
      ...
      {makeThisModal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:400,padding:16}} onClick={()=>{if(!makeThisLoading){setMakeThisModal(false);}}}>
          <div style={{background:C.card,border:"1px solid "+C.border,borderRadius:18,padding:28,maxWidth:480,width:"100%"}} onClick={e=>e.stopPropagation()}>
            ...
            {!makeThisResult?(
              <div>
                <input value={makeThisInput} onChange={e=>setMakeThisInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&makeThisInput.trim()&&!makeThisLoading&&(async()=>{setMakeThisLoading(true);const cellarInfo=await getCellarCookingBlock();const res=await callClaude({system:"Recipe AI. Return ONLY valid JSON, no markdown, no backticks.",prompt:"Give me a recipe for: "+makeThisInput.trim()+". Inventory available: "+inventory.map(i=>i.name).filter(Boolean).join(", ")+cellarInfo.block+" CRITICAL: if the requested name specifies a cooking method/technique (sous vide, air fryer, slow cooker, grilled, smoked, etc.), the instructions must actually use that exact method with correct temps/times — never keep a technique in the name while writing different, more conventional instructions. Return JSON: {name,description,time,difficulty,servings,ingredients:[{name,qty,unit} objects — qty is a NUMBER, unit is a short string like \"cup\",\"tsp\",\"tbsp\",\"oz\",\"lb\" or \"\" for countable items],instructions:[5 strings — CRITICAL: every step referencing a measured ingredient MUST use a {{ing:N}} placeholder instead of typing a number, where N is that ingredient's zero-based index in ingredients. Never type a literal quantity.],usesFromInventory:[ingredient names from inventory],missingIngredients:[items NOT in inventory]"+cellarInfo.schemaField+"}."});try{const raw=(typeof res==="string"?res:Array.isArray(res)?res.map(r=>r.text||"").join(""):res?.[0]?.text||"").replace(/```json|```/g,"").trim();const s=raw.indexOf("{"),e2=raw.lastIndexOf("}");const p=JSON.parse(raw.slice(s,e2+1));setMakeThisResult(p);}catch(e){showAlert("Could not parse recipe. Try again.");}setMakeThisLoading(false);})()}  placeholder='e.g. "Peanut butter cookies"' style={{...bInp,marginBottom:14,fontSize:15}} autoFocus/>
                <button onClick={async()=>{if(!makeThisInput.trim())return;setMakeThisLoading(true);const cellarInfo=await getCellarCookingBlock();const res=await callClaude({system:"Recipe AI. Return ONLY valid JSON, no markdown, no backticks.",prompt:"Give me a recipe for: "+makeThisInput.trim()+". Inventory available: "+inventory.map(i=>i.name).filter(Boolean).join(", ")+cellarInfo.block+" CRITICAL: if the requested name specifies a cooking method/technique (sous vide, air fryer, slow cooker, grilled, smoked, etc.), the instructions must actually use that exact method with correct temps/times — never keep a technique in the name while writing different, more conventional instructions. Return JSON: {name,description,time,difficulty,servings,ingredients:[{name,qty,unit} objects — qty is a NUMBER, unit is a short string like \"cup\",\"tsp\",\"tbsp\",\"oz\",\"lb\" or \"\" for countable items],instructions:[5 strings — CRITICAL: every step referencing a measured ingredient MUST use a {{ing:N}} placeholder instead of typing a number, where N is that ingredient's zero-based index in ingredients. Never type a literal quantity.],usesFromInventory:[ingredient names from inventory],missingIngredients:[items NOT in inventory]"+cellarInfo.schemaField+"}."});try{const raw=(typeof res==="string"?res:Array.isArray(res)?res.map(r=>r.text||"").join(""):res?.[0]?.text||"").replace(/```json|```/g,"").trim();const s=raw.indexOf("{"),e2=raw.lastIndexOf("}");const p=JSON.parse(raw.slice(s,e2+1));setMakeThisResult(p);}catch(e){showAlert("Could not parse recipe. Try again.");}setMakeThisLoading(false);}} disabled={!makeThisInput.trim()||makeThisLoading} style={{...bBtn("primary"),width:"100%",padding:"12px",fontSize:15,opacity:makeThisInput.trim()&&!makeThisLoading?1:0.5}}>{makeThisLoading?"Finding recipe...":"Find Recipe →"}</button>
              </div>
            ):(
              <div>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                ...
                <button onClick={()=>{
                  ...
                  if(missing.length===0){showAlert("You have all the ingredients on hand!");}
                    ...
                    setShopping(prev=>{
                      const toAdd=missing.filter(m=>!prev.some(p=>(p.name||"").toLowerCase()===m.name.toLowerCase()));
                      return [...prev,...toAdd.map(m=>({name:m.name,qty:m.qty||1,unit:m.unit||"",category:"Pantry",checked:false,suggestBulk:false,source:"Family Recipe: "+frViewRecipe.name}))];
                    });
                    showAlert(missing.length+" ingredient"+(missing.length!==1?"s":"")+" added to your shopping list!");
                  }
                ...
                <button onClick={()=>cookRecipe(frViewRecipe,frServings)} style={{flex:1,background:"#2aa86e",border:"none",borderRadius:10,padding:"12px",color:"#fff",fontFamily:"Georgia,serif",fontSize:seniorMode?16:13,cursor:"pointer",fontWeight:700}} disabled={isViewer}>🍳 I Made This — Update Inventory</button>
                <button onClick={()=>{
              ...
              {frPhotos.length>0&&<div style={{display:"flex",gap:8,marginTop:4}}>
                ...
                <button onClick={async()=>{
                  ...
                  setFrEditRecipe({...parsed,id:Date.now(),servings:parsedServings,rotation:false,frequency:"4week",seasons:[],photo:null});
                    ...
                    setFrPhotos([]);
                    setFrAddMode("review");
                  }catch(err){showAlert("Could not read recipe. Try clearer photos or better lighting.");}
                  setFrLoading(false);

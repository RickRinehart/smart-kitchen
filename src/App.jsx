...
Keep responses concise — 2-4 sentences max unless explaining a feature. Use plain language. No bullet points unless listing steps.`;
  ...
  const speak=(text)=>{
    ...
    const femaleNames=["Samantha","Karen","Victoria","Moira","Fiona","Tessa","Veena","Zira","Google US English Female","Microsoft Zira","female"];
    const maleNames=["Tom","Daniel","Alex","Fred","Google US English Male","Microsoft David","Microsoft Mark","male"];
    // Gender: try named voice first, fall back to pitch/rate adjustment
    if(gender==="male"){utt.pitch=0.7;utt.rate=0.88;}else{utt.pitch=1.15;utt.rate=0.95;}
    const assignVoiceAndSpeak=()=>{
      const voices=window.speechSynthesis.getVoices();
      const enVoices=voices.filter(v=>v.lang&&v.lang.startsWith("en"));
      const namedMatch=enVoices.find(v=>gender==="female"?femaleNames.some(n=>v.name.toLowerCase().includes(n.toLowerCase())):maleNames.some(n=>v.name.toLowerCase().includes(n.toLowerCase())));
      const picked=namedMatch||enVoices.find(v=>v.localService)||enVoices[0]||voices[0];

...
<input id="fr-dish-gallery" type="file" accept="image/*" style={{display:"none"}} onChange={e=>{
  ...
  e.target.value="";
    ...
    {showEmailGate&&!user&&(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:900,padding:20}}><div style={{background:C.card,borderRadius:18,padding:28,width:"100%",maxWidth:440,border:"2px solid #C8963E",boxShadow:"0 8px 40px rgba(0,0,0,0.6)"}}>{magicLinkSent?(<div style={{textAlign:"center"}}><div style={{fontSize:48,marginBottom:12}}>📧</div><div style={{fontFamily:"'DM Serif Display',serif",fontSize:24,color:"#C8963E",marginBottom:12}}>Check your email</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:15,color:C.muted,lineHeight:1.7,marginBottom:20}}>We sent a secure access link to<br/><strong style={{color:C.text}}>{trialEmail}</strong><br/>Tap the link in that email to open Smart Kitchen with full 30-day access.</div><div style={{background:C.surface,borderRadius:10,padding:"10px 14px",marginBottom:20,fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"#888",lineHeight:1.6}}>🔍 Didn\u2019t receive it? Check your spam folder. The link expires in 1 hour. If you used a typo, tap below to try again.</div><button onClick={()=>{setMagicLinkSent(false);setTrialEmail("");setTrialEmailError("");}} style={{background:"transparent",border:"1px solid #555",borderRadius:10,padding:"10px 20px",color:"#888",fontFamily:"'DM Sans',sans-serif",fontSize:13,cursor:"pointer",marginBottom:8}}>Try a different email</button><br/><button onClick={()=>setShowEmailGate(false)} style={{background:"transparent",border:"none",color:"#666",fontFamily:"'DM Sans',sans-serif",fontSize:12,cursor:"pointer",marginTop:4}}>Back to sign in</button></div>):(<div><div style={{textAlign:"center",marginBottom:20}}><div style={{fontFamily:"'DM Serif Display',serif",fontSize:28,color:"#C8963E",marginBottom:6}}>✨ Try Smart Kitchen Free</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:14,color:C.muted,lineHeight:1.6}}>Get full access to every feature for 30 days — no credit card, no password, no commitment.</div></div><div style={{background:C.surface,borderRadius:12,padding:"12px 16px",marginBottom:16}}><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"#888",letterSpacing:0.8,marginBottom:8}}>YOUR 30-DAY TRIAL INCLUDES</div><div style={{display:"flex",flexDirection:"column",gap:4}}>{["✓ 7-day AI meal planning","✓ Full inventory management","✓ Bluetooth scale + Smart Plate Mode","✓ Medical+ Intelligence (all conditions)","✓ Food Journal + Nutrition Dashboard","✓ Voice assistant, shopping list, and more"].map(f=>(<div key={f} style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:C.text}}>{f}</div>))}</div></div><div style={{marginBottom:8}}><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:"#888",letterSpacing:0.8,marginBottom:4}}>YOUR EMAIL ADDRESS</div><input value={trialEmail} onChange={e=>{setTrialEmail(e.target.value);setTrialEmailError("");}} placeholder="you@email.com" type="email" onKeyDown={e=>{if(e.key==="Enter")document.getElementById("sendMagicLinkBtn")?.click();}} style={{width:"100%",background:C.surface,border:"1px solid "+(trialEmailError?"#dc2626":"#C8963E"),borderRadius:10,padding:"12px 14px",color:C.text,fontFamily:"'DM Sans',sans-serif",fontSize:15,boxSizing:"border-box"}}/>{trialEmailError&&<div style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:"#dc2626",marginTop:4}}>{trialEmailError}</div>}</div><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:"#666",lineHeight:1.6,marginBottom:16,padding:"8px 12px",background:"#ffffff08",borderRadius:8,border:"1px solid #333"}}>🔒 <strong style={{color:"#888"}}>Privacy:</strong> Your email will only be used for necessary communications between RG Digital Labs, LLC / Smart Kitchen and you. We will never sell, share, or distribute your email address to third parties.</div><button id="sendMagicLinkBtn" onClick={async()=>{const email=trialEmail.trim();if(!email||!email.includes("@")||!email.includes(".")){setTrialEmailError("Please enter a valid email address (e.g. you@gmail.com).");return;}setTrialEmailSubmitting(true);setTrialEmailError("");try{const {error}=await supabase.auth.signInWithOtp({email,options:{emailRedirectTo:"https://smart-kitchen-opal.vercel.app",data:{trial:true,trial_source:"30day_gate",trial_start:new Date().toISOString()}}});if(error){setTrialEmailError("Could not send link: "+error.message);setTrialEmailSubmitting(false);return;}await fetch("/api/mailchimp-subscribe",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,name:"Trial User",tags:["30-day-trial","magic-link"]})}).catch(()=>{});try{localStorage.setItem("sk_guestTrialEmail",email);localStorage.setItem("sk_trialStart",Date.now().toString());}catch{}setMagicLinkSent(true);}catch(e){setTrialEmailError("Network error — check your connection and try again.");}setTrialEmailSubmitting(false);}} disabled={trialEmailSubmitting} style={{width:"100%",background:"#C8963E",border:"none",borderRadius:12,padding:"14px",color:"#000",fontFamily:"'DM Sans',sans-serif",fontSize:16,fontWeight:700,cursor:trialEmailSubmitting?"default":"pointer",marginBottom:10,opacity:trialEmailSubmitting?0.6:1}}>{trialEmailSubmitting?"⏳ Sending your link...":"📧 Send My Access Link"}</button><button onClick={()=>{setShowEmailGate(false);setTrialEmail("");setTrialEmailError("");}} style={{width:"100%",background:"transparent",border:"none",color:"#888",fontFamily:"'DM Sans',sans-serif",fontSize:13,cursor:"pointer",padding:4}}>← Back — create a full account instead</button></div>)}</div></div>)}    </div>
  );
}

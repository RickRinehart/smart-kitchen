...
Keep responses concise — 2-4 sentences max unless explaining a feature. Use plain language. No bullet points unless listing steps.`;
  ...
  const speak=(text)=>{
    ...
    const femaleNames=["Samantha","Karen","Victoria","Moira","Fiona","Tessa","Veena","Zira","Google US English Female","Microsoft Zira"];
    const maleNames=["Tom","Daniel","Alex","Fred","Google US English Male","Microsoft David","Microsoft Mark"];
    const assignVoiceAndSpeak=()=>{
      const voices=window.speechSynthesis.getVoices();
      const preferred=voices.find(v=>gender==="female"?femaleNames.some(n=>v.name.includes(n)):maleNames.some(n=>v.name.includes(n)))||voices.find(v=>v.lang==="en-US"&&v.localService)||voices[0];
    ...
    if(voices&&voices.length>0){
      ...
      // Android Chrome: voices load async, wait for voiceschanged

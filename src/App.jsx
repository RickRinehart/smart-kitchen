    ...
    if(can.medicalCompliance){
      ...
      if(medProfiles.length>0){
        s+="MEDICAL+ PROFILES: "+medProfiles.map(p=>{
          ...
          if((p.medications||[]).length>0) parts.push("Medications: "+p.medications.filter(m=>m.name).map(m=>m.name+(m.dose?" "+m.dose:"")+(m.qty?" x"+m.qty:"")+(m.freq?" "+m.freq:"")).join(", ")+" — avoid known food-drug interactions (warfarin/vitamin K, statins/grapefruit, MAOIs/tyramine, etc)");
          if((p.supplements||[]).length>0) parts.push("Supplements: "+p.supplements.filter(s=>s.name).map(s=>s.name+(s.dose?" "+s.dose:"")+(s.qty?" x"+s.qty:"")).join(", "));
                        ...
                        <div style={{marginTop:10,position:"relative"}}>
                          ...
                          <div style={{background:"#0a1628",borderRadius:10,padding:12}}>
                            ...
                            <div style={{marginBottom:8}}>
                              ...
                              {(profile.medications||[]).map((med,mi)=>(<div key={mi} style={{display:"grid",gridTemplateColumns:"1fr 70px 60px 70px auto",gap:4,marginBottom:4,alignItems:"center"}}><input style={{...bInp,fontSize:12}} placeholder="Medication name" value={med.name||""} onChange={e=>setFamilyProfiles(p=>p.map(pr=>pr.id===profile.id?{...pr,medications:pr.medications.map((m,i)=>i===mi?{...m,name:e.target.value}:m)}:pr))}/><input style={{...bInp,fontSize:12}} placeholder="Dose" value={med.dose||""} onChange={e=>setFamilyProfiles(p=>p.map(pr=>pr.id===profile.id?{...pr,medications:pr.medications.map((m,i)=>i===mi?{...m,dose:e.target.value}:m)}:pr))}/><input style={{...bInp,fontSize:12}} placeholder="Qty" value={med.qty||""} onChange={e=>setFamilyProfiles(p=>p.map(pr=>pr.id===profile.id?{...pr,medications:pr.medications.map((m,i)=>i===mi?{...m,qty:e.target.value}:m)}:pr))}/><input style={{...bInp,fontSize:12}} placeholder="Freq" value={med.freq||""} onChange={e=>setFamilyProfiles(p=>p.map(pr=>pr.id===profile.id?{...pr,medications:pr.medications.map((m,i)=>i===mi?{...m,freq:e.target.value}:m)}:pr))}/><button onClick={()=>setFamilyProfiles(p=>p.map(pr=>pr.id===profile.id?{...pr,medications:pr.medications.filter((_,i)=>i!==mi)}:pr))} style={{...bBtn("ghost"),padding:"4px 8px",color:C.red,fontSize:12}}>X</button></div>))}
                            ...
                            <div style={{marginBottom:8}}>
                              ...
                              {(profile.supplements||[]).map((sup,si)=>(<div key={si} style={{display:"grid",gridTemplateColumns:"1fr 70px 60px auto",gap:4,marginBottom:4,alignItems:"center"}}><input style={{...bInp,fontSize:12}} placeholder="Supplement name" value={sup.name||""} onChange={e=>setFamilyProfiles(p=>p.map(pr=>pr.id===profile.id?{...pr,supplements:pr.supplements.map((s,i)=>i===si?{...s,name:e.target.value}:s)}:pr))}/><input style={{...bInp,fontSize:12}} placeholder="Dose" value={sup.dose||""} onChange={e=>setFamilyProfiles(p=>p.map(pr=>pr.id===profile.id?{...pr,supplements:pr.supplements.map((s,i)=>i===si?{...s,dose:e.target.value}:s)}:pr))}/><input style={{...bInp,fontSize:12}} placeholder="Qty" value={sup.qty||""} onChange={e=>setFamilyProfiles(p=>p.map(pr=>pr.id===profile.id?{...pr,supplements:pr.supplements.map((s,i)=>i===si?{...s,qty:e.target.value}:s)}:pr))}/><button onClick={()=>setFamilyProfiles(p=>p.map(pr=>pr.id===profile.id?{...pr,supplements:pr.supplements.filter((_,i)=>i!==si)}:pr))} style={{...bBtn("ghost"),padding:"4px 8px",color:C.red,fontSize:12}}>X</button></div>))}
...
{canIHaveOpen&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:600,padding:16}} onClick={()=>setCanIHaveOpen(false)}>
        <div style={{background:"#1a1f2e",border:"1px solid #2a3042",borderRadius:16,padding:24,maxWidth:400,width:"100%",maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <span style={{fontSize:18,fontWeight:700,color:"#f472b6"}}>🔍 Can I Have This?</span>
            <button onClick={()=>setCanIHaveOpen(false)} style={{background:"transparent",border:"none",color:"#888",cursor:"pointer",fontSize:18}}>✕</button>
          </div>
          <div style={{fontSize:13,color:"#888",marginBottom:14}}>
            {restrictedProfiles.map(p=>RESTRICTION_PRESETS[p.restriction]).filter(Boolean).map((r,i)=><span key={i} style={{fontSize:10,background:r.color+"22",color:r.color,padding:"2px 8px",borderRadius:10,marginRight:4,border:"1px solid "+r.color+"44"}}>{r.icon} {r.label}</span>)}
          </div>
          ...
          {canIHaveResult&&<div>
            <div style={{background:canIHaveResult.verdict==="YES"?"#052e16":canIHaveResult.verdict==="LIMITED"?"#1c1400":"#1f0000",border:"2px solid "+(canIHaveResult.verdict==="YES"?"#22c55e":canIHaveResult.verdict==="LIMITED"?"#f59e0b":"#ef4444"),borderRadius:10,padding:"16px",marginBottom:12,textAlign:"center"}}>
              <div style={{fontSize:40}}>{canIHaveResult.verdict==="YES"?"✅":canIHaveResult.verdict==="LIMITED"?"⚠":"❌"}</div>
              <div style={{fontSize:24,fontWeight:700,color:canIHaveResult.verdict==="YES"?"#22c55e":canIHaveResult.verdict==="LIMITED"?"#f59e0b":"#ef4444"}}>{canIHaveResult.verdict}</div>
              <div style={{fontSize:12,color:"#aaa",marginTop:4}}>{canIHaveResult.flag}</div>
            </div>
            <div style={{fontSize:13,color:"#ccc",marginBottom:12}}>{canIHaveResult.explanation}</div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setCanIHaveResult(null)} style={{flex:1,padding:"10px",background:"transparent",border:"1px solid #3a3f52",borderRadius:6,color:"#888",cursor:"pointer"}}>📸 Try Again</button>
              <button onClick={()=>{setCanIHaveOpen(false);setCanIHaveResult(null);setCanIHaveImg(null);}} style={{flex:1,padding:"10px",background:"transparent",border:"1px solid #3a3f52",borderRadius:6,color:"#888",cursor:"pointer"}}>Done</button>
            </div>
          </div>}
        </div>
      </div>}
        ...
        <div style={{flex:1,overflowY:"auto",padding:"16px 16px 8px",display:"flex",flexDirection:"column",gap:12}}>
          {chatMessages.map(msg=>(
            <div key={msg.id} style={{display:"flex",justifyContent:msg.role==="user"?"flex-end":"flex-start"}}>
              <div style={{maxWidth:"82%",background:msg.role==="user"?"#C8963E":C.surface,color:msg.role==="user"?"#0c0e14":C.text,borderRadius:msg.role==="user"?"16px 16px 4px 16px":"16px 16px 16px 4px",padding:"10px 14px",fontFamily:FM,fontSize:seniorMode?17:13,lineHeight:1.6,whiteSpace:"pre-wrap"}}

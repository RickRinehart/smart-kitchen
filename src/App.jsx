                  ...
                  {mealPlan.map((day,i)=>(
                    <div key={i} style={{background:C.card,border:"1px solid "+(day.quickMeal?"#f59e0b":C.border),borderRadius:seniorMode?16:12,padding:seniorMode?18:12}}>
                      ...
                      <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                        ...
                        {familyProfiles.some(p=>p.guidedPlateMode)&&(<button onClick={()=>{const activeP=familyProfiles.find(p=>p.guidedPlateMode)||familyProfiles[0];setPlateStep(0);setPlateComponents([]);setPlateCumulativeG(0);setPlateSessionId(Date.now().toString());setPlateCoachNote("");setShowPlateSummary(false);setScaleCalcResult(null);setScaleError("");setPlateSession({active:true,memberName:activeP.name||"",mealName:day.meal,mealDay:day.day,activeProfile:activeP});setShowScaleModal(true);}} style={{background:"#10b98122",border:"1px solid #10b981",borderRadius:seniorMode?10:6,color:"#10b981",cursor:"pointer",fontFamily:FM,fontSize:seniorMode?16:11,padding:seniorMode?"10px 16px":"6px 12px",flexShrink:0,fontWeight:600}} disabled={isViewer}>🍽 Build Plate</button>)}
